...
class Hash_set {
  ...
  */
  ...
  Hash_set(PSI_memory_key psi_key, CHARSET_INFO *charset,
           ulong default_array_elements, size_t key_offset, size_t key_length,
           my_hash_get_key get_key, void (*free_element)(void *), uint flags) {
  ...
  */
  ...
  T *find(const void *key, size_t klen) const {
    return (T *)my_hash_search(&m_hash, reinterpret_cast<const uchar *>(key),
                               klen);
  }
  ...
  T *find(const T *other) const {
    size_t klen;
    const uchar *key =
        m_hash.get_key(reinterpret_cast<const uchar *>(other), &klen, false);
