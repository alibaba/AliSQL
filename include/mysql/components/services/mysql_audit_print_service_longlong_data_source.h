...
#ifndef MYSQL_AUDIT_PRINT_SERVICE_LONGLONG_DATA_SOURCE_H
#define MYSQL_AUDIT_PRINT_SERVICE_LONGLONG_DATA_SOURCE_H
...
/**
  ...
  Audit print service allows to obtain data of the long long type associated
  with the THD object, which keeps various data of the user session. It also
  allows to obtain data not necessarily bound with the user session.
  ...
  @endcode
*/
BEGIN_SERVICE_DEFINITION(mysql_audit_print_service_longlong_data_source)
...
/**
  ...
  @return
    ...
    @retval TRUE  Failed.
*/
DECLARE_BOOL_METHOD(get, (MYSQL_THD thd, my_h_string name, long long *out));

END_SERVICE_DEFINITION(mysql_audit_print_service_longlong_data_source)

#endif /* MYSQL_AUDIT_PRINT_SERVICE_LONGLONG_DATA_SOURCE_H */
