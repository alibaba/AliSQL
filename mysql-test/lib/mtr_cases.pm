...
sub process_opts_file {
  ...
  # # Add duckdb options to slave_opt, TODO: add an mtr option
  # if ($opt_name eq 'slave_opt') {
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_convert_all_at_startup=ON';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_convert_all_at_startup_ignore_error=ON';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_convert_all_at_startup_threads=4';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_convert_all_skip_mtr_db=ON';
  #   push @{ $tinfo->{$opt_name} }, '--force_innodb_to_duckdb=ON';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_copy_ddl_in_batch=ON';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_dml_in_batch=ON';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_force_no_collation=OFF';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_mode=ON';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_require_primary_key=OFF';
  #   push @{ $tinfo->{$opt_name} }, '--duckdb_update_modified_column_only=ON';
  # }
