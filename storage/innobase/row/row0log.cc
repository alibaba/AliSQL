				...
				and a PRIMARY KEY is being created) */
{
	...
	ut_ad(!rec_comfort || dict_index_need_comfort(index));
	...
	trx_id_t		trx_id)		/*!< in: DB_TRX_ID of mrec */
{
	...
	if (dict_table_is_comfort(log->table) && row) {
		dtuple_set_info_bits((dtuple_t*)row,
						...
						for parsing mrec */
{
	...
	*error = DB_SUCCESS;
	...
	case ROW_T_INSERT:
		...
		/* Important:
		   The mrec is generated directly by changed record
		   which included rec_comfort. so rec_init_offsets_temp
		   need to deal with rec_comfort. */
		rec_offs_set_n_fields(offsets, dup->index->n_fields);
		rec_init_offsets_temp(mrec, dup->index, offsets, rec_comfort);
	...
	case ROW_T_DELETE:
		...
		/* Important:
		   The delete op record is generated by pk column
		   through 'temp' so it didn't need to deal with rec_comfort. */
		rec_init_offsets_temp(mrec, new_index, offsets, false);
	...
	case ROW_T_UPDATE:
		...
		if (dup->index->online_log->same_pk) {
			...
			rec_offs_set_n_fields(offsets, dup->index->n_fields);
			/* If same PK, need to deal with rec_comfort,
			   since the log include new row.  */
			rec_init_offsets_temp(mrec, dup->index, offsets, rec_comfort);
			...
			DB_TRX_ID, DB_ROLL_PTR. */
			rec_offs_set_n_fields(offsets, new_index->n_uniq + 2);
			/* If different PK, then log include old PK and new row.
			   Old PK didn't set rec_comfort.
			   But new row need deal with rec_comfort. */
			rec_init_offsets_temp(mrec, new_index, offsets, false);
			...
			rec_offs_set_n_fields(offsets, dup->index->n_fields);
                        /* It is updating with different PK, now deal with new row. */
			rec_init_offsets_temp(mrec, dup->index, offsets, rec_comfort);
...
corrupted:
	...
	rec_init_offsets_temp(mrec, index, offsets, rec_comfort);
