...
#include "duckdb/duckdb_manager.h"
#include "duckdb/duckdb_timezone.h"  // days_at_timestart
...
DeltaAppender *DeltaAppenders::get_appender(std::string &db, std::string &tb,
  ...
  if (it != m_append_infos.end()) {
    ...
    try {
      ...
      sql_print_error(ER(ER_DUCKDB), error.RawMessage().c_str());
...
bool DeltaAppenders::flush_all(bool idempotent_flag, std::string &error_msg) {
  try {
    ...
    sql_print_information("my_duckdb: %s", error_msg.c_str());
    return true;
  }
  // remove all
...
int DeltaAppender::append_row_insert(TABLE *table, ulonglong trx_no,
  ...
  try {
    ...
    duckdb::ErrorData error(ex);
    sql_print_information("my_duckdb: %s", error.RawMessage().c_str());
    my_error(ER_DUCKDB_APPENDER_ERROR, MYF(0), error.RawMessage().c_str());
...
bool DeltaAppender::Initialize(TABLE *table) {
  if (m_use_tmp_table) {
    bitmap_init(&m_pk_bitmap, nullptr, table->s->fields, false);
...
int DeltaAppender::append_mysql_field(Field *field,
                                      MY_BITMAP *blob_type_map) {
  ...
  switch (type) {
    case MYSQL_TYPE_TINY: {
      longlong value = ((Field_tiny *)field)->val_int();
      appender->Append<int64_t>(value);
      break;
    }

    case MYSQL_TYPE_SHORT: {
      longlong value = ((Field_short *)field)->val_int();
      appender->Append<int64_t>(value);
      break;
    }

    case MYSQL_TYPE_INT24: {
      longlong value = ((Field_medium *)field)->val_int();
      appender->Append<int64_t>(value);
      break;
    }

    case MYSQL_TYPE_LONG: {
      longlong value = ((Field_long *)field)->val_int();
      appender->Append<int64_t>(value);
      break;
    }

    case MYSQL_TYPE_LONGLONG: {
      longlong value = ((Field_longlong *)field)->val_int();
      if ((field->key_type() == HA_KEYTYPE_BINARY) || (field->key_type() == HA_KEYTYPE_ULONGLONG)) {
    ...
    case MYSQL_TYPE_FLOAT: {
      double value = ((Field_float *)field)->val_real();
      // write_batch_double(col_index, &value);
      appender->Append<double>(value);
      break;
    }
    case MYSQL_TYPE_DOUBLE: {
      double value = ((Field_double *)field)->val_real();
      // write_batch_double(col_index, &value);
      appender->Append<double>(value);
      break;
    }
    ...
    case MYSQL_TYPE_NEWDECIMAL: {
      ...
      if (precision <= 38) {
        ...
        if (value.intg + value.frac > (int)precision || value.frac > (int)dec) {
          sql_print_information("Append DECIMAL field failed!");
        ...
        double value = ((Field_new_decimal *)field)->val_real();
        appender->Append<double>(value);
      }
      break;
    }
    ...
    case MYSQL_TYPE_NEWDATE: {
      ...
      MYSQL_TIME tm;
      ((Field_newdate *)field)->get_date(&tm, TIME_FUZZY_DATE);
      long date =
    ...
    case MYSQL_TYPE_DATETIME2: {
      MYSQL_TIME tm;
      ((Field_datetimef *)field)->get_date(&tm, TIME_FUZZY_DATE);
      my_bool not_used;
    ...
    case MYSQL_TYPE_YEAR: {
      longlong value = ((Field_year *)field)->val_int();
      appender->Append<int64_t>(value);
      break;
    }
    ...
    case MYSQL_TYPE_TIMESTAMP2: {
      ...
      appender->Append<duckdb::timestamp_t>(
          duckdb::timestamp_t{tm.tv_sec * 1000000 + tm.tv_usec});
      break;
    }
