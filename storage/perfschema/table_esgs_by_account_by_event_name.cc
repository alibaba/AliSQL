...
int table_esgs_by_account_by_event_name::delete_all_rows() {
  reset_events_stages_by_thread();
  reset_events_stages_by_account();
  return 0;
}

ha_rows table_esgs_by_account_by_event_name::get_row_count() {
...
void table_esgs_by_account_by_event_name::reset_position() {
  m_pos.reset();
  m_next_pos.reset();
}
...
int table_esgs_by_account_by_event_name::rnd_next() {
  PFS_account *account;
...
int table_esgs_by_account_by_event_name::index_next() {
  PFS_account *account;
  PFS_stage_class *stage_class;
  bool has_more_account = true;
