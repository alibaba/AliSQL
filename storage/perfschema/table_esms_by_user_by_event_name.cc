...
Plugin_table table_esms_by_user_by_event_name::m_table_def(
    /* Schema name */
    "performance_schema",
    /* Name */
...
bool PFS_index_esms_by_user_by_event_name::match(PFS_user *pfs) {
  if (m_fields >= 1) {
...
bool PFS_index_esms_by_user_by_event_name::match(PFS_instr_class *instr_class) {
        ...
        do {
          ...
          if (statement_class) {
            if (m_opened_index->match(statement_class)) {
              if (!make_row(user, statement_class)) {
                m_next_pos.set_after(&m_pos);
                return 0;
              }
            }
...
int table_esms_by_user_by_event_name::make_row(PFS_user *user,
...
int table_esms_by_user_by_event_name::read_row_values(TABLE *table,
                                                      unsigned char *buf,
                                                      Field **fields,
                                                      bool read_all) {
  Field *f;
