...
int table_ets_by_thread_by_event_name::delete_all_rows() {
  reset_events_transactions_by_thread();
  return 0;
}

ha_rows table_ets_by_thread_by_event_name::get_row_count() {
...
void table_ets_by_thread_by_event_name::reset_position() {
  m_pos.reset();
  m_next_pos.reset();
}
...
int table_ets_by_thread_by_event_name::rnd_next() {
  PFS_thread *thread;
...
int table_ets_by_thread_by_event_name::index_next() {
  PFS_thread *thread;
  PFS_transaction_class *transaction_class;
  bool has_more_thread = true;
