 ...
 * 7 - interpreted scan
 */
static const char *g_ops[] = {"serial pk",
                              "batch pk",
...
int drop_table() {
  if (!g_paramters[P_CREATE].value) return 0;
...
int load_table() {
  if (!g_paramters[P_LOAD].value) return 0;
...
void print_result() {
  ...
  for (int i = 0; i < P_OP_TYPES; i++) {
    g_err << g_ops[i] << " avg: " << (int)((1000 * g_times[i]) / tmp)
          << " us/row (" << (1000 * tmp) / g_times[i] << " rows / sec)" << endl;
  }
}
