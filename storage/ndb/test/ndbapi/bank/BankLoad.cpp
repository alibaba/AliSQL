...
const AccountTypesStruct accountTypes[] = {{0, "KASSA"},
                                           {1, "BANKOMAT"},
                                           {2, "POSTGIRO"},
                                           {3, "LONEKONTO"},
                                           {4, "SPARKONTO"}};

const int accountTypesSize = sizeof(accountTypes) / sizeof(AccountTypesStruct);

const char *tableNames[] = {"GL", "ACCOUNT", "SYSTEM_VALUES", "TRANSACTION",
                            "ACCOUNT_TYPE"};

const int tableNamesSize = sizeof(tableNames) / sizeof(const char *);

int Bank::getNumAccountTypes() { return accountTypesSize; }

int Bank::createAndLoadBank(bool ovrWrt, bool disk, int num_accounts) {
  m_ndb.init();
  if (m_ndb.waitUntilReady() != 0) return NDBT_FAILED;
  ...
  if (!m_skip_create && createTables(disk) != NDBT_OK) return NDBT_FAILED;

  if (clearTables() != NDBT_OK) return NDBT_FAILED;

  if (loadAccountType() != NDBT_OK) return NDBT_FAILED;

  if (loadAccount(num_accounts) != NDBT_OK) return NDBT_FAILED;

  if (loadSystemValues() != NDBT_OK) return NDBT_FAILED;

  if (loadGl() != NDBT_OK) return NDBT_FAILED;
...
int Bank::dropBank() {
  ...
  if (dropTables() != NDBT_OK) return NDBT_FAILED;
...
int Bank::createTables(bool disk) {
  for (int i = 0; i < tableNamesSize; i++) {
    if (createTable(tableNames[i], disk) != NDBT_OK) return NDBT_FAILED;
  }
  return NDBT_OK;
}
...
int Bank::dropTables() {
  for (int i = 0; i < tableNamesSize; i++) {
    if (dropTable(tableNames[i]) != NDBT_OK) return NDBT_FAILED;
  }
  return NDBT_OK;
}
...
int Bank::clearTables() {
  for (int i = 0; i < tableNamesSize; i++) {
    if (clearTable(tableNames[i]) != NDBT_OK) return NDBT_FAILED;
  }
  return NDBT_OK;
}
...
int Bank::clearTable(const char *tabName) {
  ...
  if (util.clearTable(&m_ndb, 64) != 0) return NDBT_FAILED;
  return NDBT_OK;
}
...
int Bank::createTable(const char *tabName, bool disk) {
  ...
  if (pTab == NULL) return NDBT_FAILED;

  const NdbDictionary::Table *org = m_ndb.getDictionary()->getTable(tabName);
  ...
  if (disk) {
    ...
    if (m_ndb.getDictionary()->createTable(copy) == -1) {
      ndbout << "Failed to create table: " << m_ndb.getNdbError() << endl;
      return NDBT_FAILED;
    }
  } else {
...
int Bank::getBalanceForAccountType(const Uint32 accountType, Uint32 &balance) {
  int check;
...
int Bank::loadAccount(int numAccounts) {
  ...
  for (int i = 0; i < numAccounts; i++) {
    ...
    // check = pOp->setValue("ACCOUNT_TYPE", (const
    // char*)(Uint16)(i/accountTypesSize), 2); NOTE attribute now changed to be
    // a 32 bit
    ...
    if (i == 0)
      ...
      accountType =
          ((i % accountTypesSize) == 0 ? 1 : (i % getNumAccountTypes()));
...
int Bank::getNumAccounts() {
  ...
  if (util.selectCount(&m_ndb, 64, &m_maxAccount) != 0) return NDBT_FAILED;
  return NDBT_OK;
}

int Bank::getMaxAmount() { return 10000; }
