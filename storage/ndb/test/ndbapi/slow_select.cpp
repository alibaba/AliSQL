  ...
  while (true) {
    ...
    while (F_sz > 0) {
      ...
      for (i = 0; i < prev_F_sz; i++) {
        ...
        if (metaid > max_val) {
          for (j = 0; j < Q_sz; j++) nextF[F_sz++] = Q[j];
          require(F_sz <= cnt);
          Q_sz = 0;
          max_val = metaid;
        }
