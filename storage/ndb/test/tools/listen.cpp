...
static void do_equal(NdbOperation *op, NdbEventOperation *pOp) {
  ...
  for (i = 0; i < n_columns; i++) {
    if (tab->getColumn(i)->getPrimaryKey() && op->equal(i, ev[i]->aRef())) {
      abort();
    }
  }
}

static void do_set_value(NdbOperation *op, NdbEventOperation *pOp) {
  struct Table_info *ti = (struct Table_info *)pOp->getCustomData();
  Vector<NdbRecAttr *> &ev = event_values[ti->id];
  const NdbDictionary::Table *tab = pOp->getTable();
  unsigned i, n_columns = tab->getNoOfColumns();
  for (i = 0; i < n_columns; i++) {
    if (!tab->getColumn(i)->getPrimaryKey() && op->setValue(i, ev[i]->aRef())) {
      abort();
    }
  }
}
...
int main(int argc, const char **argv) {
  ...
  struct getargs args[] = {{"connectstring1", 'c', arg_string, &connectstring1,
                            "connectstring1", ""},
                           {"connectstring2", 'C', arg_string, &connectstring2,
                            "connectstring2", ""},
  ...
  for (i = optind; i < argc; i++) {
    ...
    myEvent->setReport((NdbDictionary::Event::EventReport)(
        NdbDictionary::Event::ER_UPDATED | NdbDictionary::Event::ER_DDL));

    if (myDict->createEvent(*myEvent)) {
      if (myDict->getNdbError().classification ==
    ...
    for (int a = 0; a < table->getNoOfColumns(); a++) {
      event_values[sz].push_back(pOp->getValue(table->getColumn(a)->getName()));
      event_pre_values[sz].push_back(
  ...
  while (true) {
    ...
    while (pOp) {
      ...
      if (ndb2) do_begin(ndb2, trans_arg);
      do {
        switch (pOp->getEventType()) {
          case NdbDictionary::Event::TE_INSERT:
            cnt_i++;
            if (ndb2) do_insert(trans_arg, pOp);
            break;
          case NdbDictionary::Event::TE_DELETE:
            cnt_d++;
            if (ndb2) do_delete(trans_arg, pOp);
            break;
          case NdbDictionary::Event::TE_UPDATE:
            cnt_u++;
            if (ndb2) do_update(trans_arg, pOp);
            break;
          case NdbDictionary::Event::TE_CLUSTER_FAILURE:
            ndbout_c("Received event: %s",
                     event_name(pOp->getEventType(), buf));
            break;
          case NdbDictionary::Event::TE_ALTER:
            ndbout_c("Received event: %s",
                     event_name(pOp->getEventType(), buf));
      ...
      if (ndb2) do_commit(trans_arg);
      ndbout_c("GCI: %u/%u events: %lld(I) %lld(U) %lld(D)", Uint32(gci >> 32),
               Uint32(gci), cnt_i, cnt_u, cnt_d);
    }
  }
end:
  for (i = 0; i < (int)event_ops.size(); i++)
