...
void Driver::initProperties() {
  ...
  if (warmupRuns < 0) {
    msg << "[ignored] warmupRuns:        '" << toString(props[L"warmupRuns"])
        << "'" << endl;
    warmupRuns = 0;
  }
...
void Driver::openLogFile() {
  cout << endl << "opening results file:" << flush;
...
void Driver::closeLogFile() {
  cout << endl << "closing results file:" << flush;
  log.close();
  cout << "           [ok: " << logFileName << "]" << endl;
}
...
void Driver::writeLogBuffers() {
  log << descr << ", rtime[ms]" << header.rdbuf()->str() << endl
      << rtimes.rdbuf()->str() << endl;
}

void Driver::begin(const string &name) {
  cout << endl;
  cout << name << endl;

  if (gettimeofday(&t0, NULL) != 0) ABORT_GETTIMEOFDAY_ERROR();
}

void Driver::finish(const string &name) {
  if (gettimeofday(&t1, NULL) != 0) ABORT_GETTIMEOFDAY_ERROR();

  const long r_usec =
      (((t1.tv_sec - t0.tv_sec) * 1000000) + (t1.tv_usec - t0.tv_usec));
  const long r_msec = r_usec / 1000;

  cout << "tx real time:                   " << r_msec << "\tms" << endl;
  rtimes << "\t" << r_msec;
  rta += r_msec;
