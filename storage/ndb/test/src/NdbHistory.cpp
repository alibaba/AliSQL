...
WorkerIdentifier::WorkerIdentifier() : m_totalWorkers(0), m_nextWorker(0) {}

void WorkerIdentifier::init(const Uint32 totalWorkers) {
...
Uint32 WorkerIdentifier::getTotalWorkers() const { return m_totalWorkers; }

Uint32 WorkerIdentifier::getNextWorkerId() {
  Uint32 r;
  lock();
  {
...
void EpochRange::dump() const {
  ndbout_c("[%u/%u,%u/%u)", hi(m_start), lo(m_start), hi(m_end), lo(m_end));
}

bool NdbHistory::RecordState::equal(const RecordState &other) const {
  if (m_state != other.m_state) {
    return false;
  }
  if (m_state == RS_EXISTS && m_updatesValue != other.m_updatesValue) {
    return false;
  }
  return true;
}

NdbHistory::Version::Version(RecordRange range) : m_range(range) {
  m_states = new RecordState[m_range.m_len];
  memset(m_states, 0, (sizeof(RecordState) * m_range.m_len));
}

NdbHistory::Version::Version(const Version *other) : m_range(other->m_range) {
  m_states = new RecordState[m_range.m_len];
...
void NdbHistory::Version::assign(const Version *other) {
...
void NdbHistory::Version::setRows(const Uint32 start, const Uint32 updatesValue,
                                  const Uint32 len) {
  setRowsImpl(start, RecordState::RS_EXISTS, updatesValue, len);
}

void NdbHistory::Version::clearRows(const Uint32 start, const Uint32 len) {
  setRowsImpl(start, RecordState::RS_NOT_EXISTS, 0, len);
}

Uint32 NdbHistory::Version::diffRowCount(const Version *other) const {
...
bool NdbHistory::Version::equal(const Version *other) const {
  return (diffRowCount(other) == 0);
}

/* dumpV print helper */
static void dumpV(const char *indent, const Uint32 start, const Uint32 end,
                  const NdbHistory::RecordState *rs) {
  if (rs->m_state == NdbHistory::RecordState::RS_NOT_EXISTS) {
    ndbout_c("%s  r %5u -> %5u : -", indent, start, end);
  } else {
    ndbout_c("%s  r %5u -> %5u : %u", indent, start, end, rs->m_updatesValue);
  }
}

void NdbHistory::Version::dump(bool full, const char *indent) const {
  ndbout_c("%sRange start %u len %u", indent, m_range.m_start, m_range.m_len);
  ...
  for (Uint32 i = 0; i < m_range.m_len; i++) {
    ...
    if (full) {
      ...
      dumpV(indent, offset + i, offset + i, rs);
    } else {
      if (i > 0) {
        ...
        if (!same) {
          ...
          dumpV(indent, offset + sameStart, offset + i - 1, prev);
      ...
      if (last) {
        ...
        dumpV(indent, offset + sameStart, offset + i, rs);
      }
    }
  }
}

void NdbHistory::Version::dumpDiff(const Version *other) const {
  assert(m_range.m_start == other->m_range.m_start);
  assert(m_range.m_len == other->m_range.m_len);
  ...
  for (Uint32 i = 0; i < m_range.m_len; i++) {
    if (m_states[i].equal(other->m_states[i])) {
      dumpV("      ", offset + i, offset + i, &m_states[i]);
    } else {
      dumpV("DIFF A", offset + i, offset + i, &m_states[i]);
      dumpV("DIFF B", offset + i, offset + i, &other->m_states[i]);
    }
  }
}

void NdbHistory::Version::setRowsImpl(const Uint32 start, const Uint32 rowState,
...
const char *NdbHistory::getVersionTypeName(const VersionType vt) {
  switch (vt) {
    case VT_LATEST:
      return "VT_LATEST";
    case VT_END_OF_GCI:
      return "VT_END_OF_GCI";
    case VT_END_OF_EPOCH:
      return "VT_END_OF_EPOCH";
    case VT_OTHER:
      return "VT_OTHER";
    default:
...
void NdbHistory::VersionMeta::dump() const {
  ndbout_c("  -- VERSION %llu %s %u/%u --", m_number,
           getVersionTypeName(m_type), Uint32(m_latest_epoch >> 32),
           Uint32(m_latest_epoch & 0xffffffff));
}

NdbHistory::NdbHistory(const Granularity granularity, const RecordRange range)
    : m_granularity(granularity), m_range(range), m_nextNumber(0) {
...
bool NdbHistory::checkVersionBoundary(const Uint64 epoch,
  ...
  const bool sameGci = ((epoch >> 32) == (lastEpoch >> 32));
...
void NdbHistory::commitVersion(const Version *version, Uint64 commitEpoch) {
  ...
  bool newVersion = checkVersionBoundary(commitEpoch, lastVersionType);
  ...
  if (newVersion) {
    ...
    if (false) {
      ndbout_c(
          ...
          "prev epoch type %s",
          m_nextNumber + 1, Uint32(lastVersion.m_meta.m_latest_epoch >> 32),
          Uint32(lastVersion.m_meta.m_latest_epoch & 0xffffffff),
          Uint32(commitEpoch >> 32), Uint32(commitEpoch & 0xffffffff),
...
const NdbHistory::Version *NdbHistory::getLatestVersion() const {
  return m_storedVersions.back().m_version;
}

const NdbHistory::Version *NdbHistory::findFirstClosestMatch(
    const Version *match, VersionMeta &vm) const {
...
NdbHistory::VersionIterator::VersionIterator(const NdbHistory &history)
    : m_history(history), m_index(0) {}

const NdbHistory::Version *NdbHistory::VersionIterator::next(VersionMeta &vm) {
...
void NdbHistory::VersionIterator::reset() { m_index = 0; }

NdbHistory::VersionMatchIterator::VersionMatchIterator(
    const NdbHistory &history, const Version *match)
    : m_vi(history), m_match(match) {
  assert(history.m_range.m_start == match->m_range.m_start);
  assert(history.m_range.m_len == match->m_range.m_len);
}

const NdbHistory::Version *NdbHistory::VersionMatchIterator::next(
    VersionMeta &vm) {
  const Version *v;
...
void NdbHistory::VersionMatchIterator::reset() { m_vi.reset(); }

NdbHistory::MatchingEpochRangeIterator::MatchingEpochRangeIterator(
    const NdbHistory &history, const Version *match)
    : m_vi(history), m_match(match) {
  assert(history.m_range.m_start == match->m_range.m_start);
  assert(history.m_range.m_len == match->m_range.m_len);
...
bool NdbHistory::MatchingEpochRangeIterator::next(EpochRange &er) {
  const Version *v;
  VersionMeta vm;
  ...
  while ((v = m_vi.next(vm)) != NULL) {
    if (m_match->equal(v)) {
      ...
      if (matchStart != NULL) {
        ...
        if (vm.m_latest_epoch == matchStartVm.m_latest_epoch) {
...
void NdbHistory::MatchingEpochRangeIterator::reset() { m_vi.reset(); }

const char *NdbHistory::getGranularityName(const Granularity gr) {
  switch (gr) {
    case GR_LATEST_ONLY:
      return "GR_LATEST_ONLY";
    case GR_LATEST_GCI:
      return "GR_LATEST_GCI";
    case GR_LATEST_GCI_EPOCH:
      return "GR_LATEST_GCI_EPOCH";
    case GR_ALL:
      return "GR_ALL";
    default:
...
void NdbHistory::dumpClosestMatch(const Version *target) const {
  /* Dump some useful info about an attempted match */
  const Version *closestMatch = NULL;
  NdbHistory::VersionMeta closestMatchMeta;

  if ((closestMatch = findFirstClosestMatch(target, closestMatchMeta))) {
