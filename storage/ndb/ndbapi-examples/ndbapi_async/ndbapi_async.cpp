...
static void callback(int result, NdbTransaction *trans, void *aObject) {
  ...
  if (result < 0) {
    ...
    if (retryable && cbData->retries++ >= MAX_RETRIES) {
      while (populate(cbData->ndb, cbData->data, cbData) < 0) milliSleep(10);
    } else {
...
bool asynchErrorHandler(NdbTransaction *trans, Ndb *ndb) {
  ...
  switch (error.status) {
    ...
    case NdbError::TemporaryError:
       ...
       */
      if (error.classification == NdbError::InsufficientSpace) return false;
      milliSleep(10);
      tempErrors++;
      return true;
      break;
    ...
    case NdbError::PermanentError:
      switch (error.code) {
        ...
        case 250:
          milliSleep(10);
          return true;  // SCAN errors that can be retried. Requires restart of
                        // scan.
        default:
          break;
      }
