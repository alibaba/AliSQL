...
static int randRange(Uint32 min, Uint32 max) {
  float r = rand();
  float f = (max - min + 1);
...
static int randRange(const Uint32 odds[], Uint32 count) {
...
int main(int argc, char **argv) {
  ...
  ndbout_c("Connected as block=%d node=%d", refToBlock(ss.getOwnRef()),
  ...
  for (Uint32 i = 0; i < 70; i++) sec0[i] = i;

  for (Uint32 i = 0; i < 123; i++) sec1[i] = 70 + i;

  for (Uint32 i = 0; i < 10; i++) sec2[i] = (i + 1) * (i + 1);
  ...
  signal2.theData[theDataLen + idx] = 0;
  idx++;
  signal2.theData[theDataLen + idx] = 1;
  idx++;
  // signal2.theData[theDataLen + idx] = 2; idx++;
  signal2.theData[theDataLen + idx] = 7;
  idx++;  // FragmentId
...
void runTest(SignalSender &ss, Uint32 count, bool verbose) {
  SimpleSignal sig;
  Uint32 sec0[256];
  Uint32 sec1[256];
  Uint32 sec2[256];
      ...
      if (test < 5) {
        ...
        while (nodes > 0) {
          ...
          if (ret->header.m_fragmentInfo == 0) {
            ...
            for (Uint32 i = 0; i < ret->header.m_noOfSections; i++)
              if (ret->theData[i] != 3) sum2 += ret->ptr[i].sz;
          }
          if (ret->header.m_fragmentInfo == 0 ||
        ...
        if (verbose) ndbout_c("done sum=%d sum2=%d", sum, sum2);
      }
    }
  }
}
