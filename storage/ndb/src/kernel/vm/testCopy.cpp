...
template <Uint32 LEN>
inline void MEMCOPY(Uint32 *to, const Uint32 *from) {
  Uint32 t0;
  Uint32 t1;
  Uint32 t2;
  Uint32 t3;
...
inline void MEMCOPY_NO_WORDS(Uint32 *to, const Uint32 *from, Uint32 len) {
  Uint32 t0;
  Uint32 t1;
  Uint32 t2;
  Uint32 t3;
...
inline void copy1(Uint32 i, TestSignal &ts) {
...
inline void copy2(Uint32 i, TestSignal &ts) {
  TestSignal &dst = g_jobBuffer[i];
...
inline void copy3(Uint32 i, TestSignal &ts) {
  TestSignal &dst = g_jobBuffer[i];

  dst.head = ts.head;
}

inline void copy4(Uint32 i, TestSignal &ts) {
  TestSignal &dst = g_jobBuffer[i];

  memcpy(&dst.head.data[0], &ts.head.data[0], sizeof(Data7));
}

inline void copy5(Uint32 i, TestSignal &ts) {
  TestSignal &dst = g_jobBuffer[i];

  MEMCOPY_NO_WORDS(&dst.head.data[0], &ts.head.data[0], 7);
}

inline void copy6(Uint32 i, TestSignal &ts) {
  TestSignal &dst = g_jobBuffer[i];

  MEMCOPY<7>(&dst.head.data[0], &ts.head.data[0]);
}

inline void copy7(Uint32 i, TestSignal &ts) {
  TestSignal &dst = g_jobBuffer[i];
...
int doTime(Uint32 ms) {
  Uint64 ms1, ms2;
  const Uint32 count = g_count;
  for (Uint32 i = 0; i < count; i++) C(i, g_signal);
  for (Uint32 i = 0; i < count; i++) C(i, g_signal);
...
void doCopyLap(Uint32 laps, const char *title) {
  ...
  ms2 -= ms1;
  Uint32 diff = ms2;
  ndbout_c("%s : %d laps in %d millis => %d copies/sec", title, div, diff,
           (1000 * div * g_count + (diff / 2)) / diff);
}

int main(int argc, const char **argv) {
  ndb_init();
  if (argc > 1) g_count = atoi(argv[1]);

  if (argc > 2) g_time = atoi(argv[2]);

  ndbout_c("Using %d entries => %zu kB ", g_count,
