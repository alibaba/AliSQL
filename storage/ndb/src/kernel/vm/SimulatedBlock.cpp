...
void SimulatedBlock::setOverloadStatus(OverloadStatus new_status) {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::set_watchdog_counter() {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::progError(int line, int err_code, const char *extra,
...
#ifdef VM_TRACE
bool SimulatedBlock::debugOutOn() {
...
void SimulatedBlock::print_static_distr_info(
  ...
  for (Uint32 ldm = 0; ldm < num_ldm_instances; ldm++) {
    g_eventLogger->info("LDM Group %u contains LDM thread %u", ldm, ldm + 1);
    Uint32 first_query_instance = (ldm + 1) * m_num_query_thread_per_ldm;
    for (Uint32 qt = 0; qt < m_num_query_thread_per_ldm; qt++) {
      g_eventLogger->info("LDM Group %u contains Query thread %u", ldm,
                          first_query_instance + qt);
    }
  }
...
void SimulatedBlock::print_debug_sched_stats(
  ...
  deb_count = 0;
  g_eventLogger->info(
      "LQHKEYREQ LDM: %llu, LQHKEYREQ LQ: %llu"
      ", LQHKEYREQ RR: %llu",
      handle->m_lqhkeyreq_ldm, handle->m_lqhkeyreq_lq, handle->m_lqhkeyreq_rr);
  g_eventLogger->info(
      "SCAN_FRAGREQ LDM %llu, SCAN_FRAGREQ LQ: %llu"
      ", SCAN_FRAGREQ RR: %llu",
      handle->m_scan_fragreq_ldm, handle->m_scan_fragreq_lq,
      handle->m_scan_fragreq_rr);
  g_eventLogger->info(
      "LQHKEYREQ LDM instances %u %u %u %u %u %u %u %u",
  ...
  g_eventLogger->info(
      "LQHKEYREQ QT instances %u %u %u %u %u %u %u %u",
  ...
  g_eventLogger->info(
      "SCAN_FRAGREQ LDM instances %u %u %u %u %u %u %u %u",
  ...
  g_eventLogger->info(
      "SCAN_FRAGREQ QT instances %u %u %u %u %u %u %u %u",
  ...
  for (Uint32 j = 1; j <= 8; j++) {
    ...
    g_eventLogger->info(
        "LDM(%u) total: %llu, %llu %llu %llu %llu %llu"
        " %llu %llu %llu %llu %llu",
        j, *total_words, est_stats[0], est_stats[1], est_stats[2], est_stats[3],
        est_stats[4], est_stats[5], est_stats[6], est_stats[7], est_stats[8],
        est_stats[9]);
    *total_words = 0;
  ...
  for (Uint32 j = 1; j <= 8; j++) {
    ...
    g_eventLogger->info(
        "QT(%u) total: %llu, %llu %llu %llu %llu %llu"
        " %llu %llu %llu %llu %llu",
        j, *total_words, est_stats[0], est_stats[1], est_stats[2], est_stats[3],
        est_stats[4], est_stats[5], est_stats[6], est_stats[7], est_stats[8],
        est_stats[9]);
...
Uint32 SimulatedBlock::get_lqhkeyreq_ref(DistributionHandler *const handle,
      ...
      (struct RoundRobinInfo *)&handle->m_rr_info[rr_group];
  {
    ...
    if (current_weight_count < curr_weight) {
       ...
       */
      Uint32 ref = numberToRef(DBLQH, ldm_instance_no, getOwnNodeId());
#ifdef DEBUG_SCHED_STATS
   ...
   */
  {
    ...
    if (current_stolen_count <= max_count) {
      ...
      Uint32 ref = numberToRef(DBQLQH, ldm_instance_no, getOwnNodeId());
      q_state->m_current_stolen_lqhkeyreq =
          current_stolen_count + load_indicator;
#ifdef DEBUG_SCHED_STATS
...
Uint32 SimulatedBlock::get_scan_fragreq_ref(DistributionHandler *const handle,
  ...
  }
  {
    ...
    if (current_stolen_count <= max_count) {
      assert(load_indicator > 0);
      Uint32 ref = numberToRef(DBQLQH, ldm_instance_no, getOwnNodeId());
      q_state->m_current_stolen_scan_fragreq =
          current_stolen_count + load_indicator;
#ifdef DEBUG_SCHED_STATS
