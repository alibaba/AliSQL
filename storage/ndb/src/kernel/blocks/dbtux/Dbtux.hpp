...
#define NullTupLoc TupLoc()
 ...
 private:
  ...
  void unpackBound(Uint32 *const outputBuffer, const ScanBound &bound,
                   KeyBoundC &searchBound);
  void findFrag(EmulatedJamBuffer *jamBuf, const Index &index, Uint32 fragId,
                FragPtr &fragPtr);
 ...
 private:
  ...
  void relinkScan(ScanOp &, Uint32 scanInstance, Frag &, bool need_lock = true,
                  Uint32 line = 0);
...
inline Dbtux::DescPage::DescPage() : m_nextPage(RNIL), m_numFree(ZNIL) {
...
inline Dbtux::ScanBound::ScanBound() : m_head(), m_cnt(0), m_side(0) {}

inline Dbtux::ScanOp::ScanOp()
...
inline Dbtux::Frag::Frag(ScanOp_pool &scanOpPool)
    : m_tableId(RNIL),
      m_indexId(RNIL),
      m_fragId(ZNIL),
      m_tree(),
...
inline Dbtux::DescHead &Dbtux::getDescHead(const Index &index) {
  DescPagePtr pagePtr;
...
inline void Dbtux::getTupAddr(const Frag &frag, TreeEnt ent, Uint32 &lkey1,
                              Uint32 &lkey2) {
  const Uint32 tableFragPtrI = frag.m_tupTableFragPtrI;
  const TupLoc tupLoc = ent.m_tupLoc;
  c_tup->tuxGetTupAddr(tableFragPtrI, tupLoc.getPageId(),
                       tupLoc.getPageOffset(), lkey1, lkey2);
  jamEntryDebug();
}
