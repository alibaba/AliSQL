...
#define BITSENUMS(name, pos, bits) \
  name##_POS = (pos), name##_BITS = (bits), name##_MASK = (1 << (bits)) - 1
#define GETBITS(name, value) (((value) >> name##_POS) & name##_MASK)
#define SETBITS(name, value, bits) \
  (((value) & ~(name##_MASK << name##_POS)) | ((bits) << name##_POS))
...
class Container {
 ...
 public:
  enum {
    CONTAINERS_PER_PAGE = 72,
    MAX_CONTAINER_INDEX = CONTAINERS_PER_PAGE - 1,
    NO_CONTAINER_INDEX = CONTAINERS_PER_PAGE
  };
  enum { HEADER_SIZE = 2, DOWN_LIMIT = 12, UP_LIMIT = 14, CONTAINER_SIZE = 28 };
};
...
inline Container::Header::Header() : m_header(~0U) {}

inline Container::Header::Header(Uint32 const header) : m_header(header) {}

inline Container::Header::Header(Header const &header) : m_header(header) {}

inline Container::Header &Container::Header::operator=(Header const &header) {
  m_header = header.m_header;
  return *this;
}

inline Container::Header::operator Uint32 const &() const { return m_header; }

inline Container::Header &Container::Header::setHeader() {
  m_header = SETBITS(HEADER, m_header, 1);
  return *this;
}

inline Container::Header &Container::Header::clearHeader() {
  m_header = SETBITS(HEADER, m_header, 0);
  return *this;
}

inline bool Container::Header::isInUse() const { return isHeader(); }

inline bool Container::Header::isFree() const { return !isHeader(); }

inline bool Container::Header::isHeader() const {
...
inline Uint32 Container::Header::getLength() const {
  return GETBITS(LENGTH, m_header);
}

inline Uint32 Container::Header::getNextEnd() const {
  assert(isInUse());
  return GETBITS(NEXT_END, m_header);
}

inline bool Container::Header::haveNext() const {
  assert(isInUse());
...
inline Uint32 Container::Header::getNextIndexNumber() const {
  assert(isInUse());
  return GETBITS(NEXT_INDEX, m_header);
}

inline bool Container::Header::isUsingBothEnds() const {
  assert(isInUse());
  return GETBITS(USING_BOTH_ENDS, m_header);
}

inline bool Container::Header::isNextOnSamePage() const {
  assert(isInUse());
  return GETBITS(NEXT_ON_SAME_PAGE, m_header);
}

inline Uint32 Container::Header::getScanBits() const {
  assert(isInUse());
  return GETBITS(SCAN_BITS, m_header);
}

inline bool Container::Header::isScanInProgress() const {
  assert(isInUse());
  return GETBITS(SCAN_IN_PROGRESS, m_header);
}

inline Container::Header &Container::Header::clearUsingBothEnds() {
  assert(isInUse());
  m_header = SETBITS(USING_BOTH_ENDS, m_header, 0);
  return *this;
}

inline Container::Header &Container::Header::setUsingBothEnds() {
  assert(isInUse());
...
inline Container::Header &Container::Header::initFree() {
...
inline Container::Header &Container::Header::initInUse() {
  m_header = 0;  // clear all (unused) bits
  setHeader();
...
inline Container::Header &Container::Header::setLength(Uint32 length) {
  assert(length <= CONTAINER_SIZE);
  m_header = SETBITS(LENGTH, m_header, length);
  return *this;
}

inline Container::Header &Container::Header::setNext(Uint32 end, Uint32 index,
                                                     bool onsamepage) {
  assert(isInUse());
  assert(end < 3);
...
inline Container::Header &Container::Header::clearNext() {
  assert(isInUse());
  return setNext(0, 0, false);
}

inline bool Container::Header::haveNextFree() const {
  assert(isFree());
  return getNextFree() <= MAX_CONTAINER_INDEX;
}

inline Container::Header &Container::Header::copyScanBits(Uint32 scanmask) {
  assert(isInUse());
...
inline Container::Header &Container::Header::setScanBits(Uint32 scanmask) {
  assert(isInUse());
...
inline Container::Header &Container::Header::clearScanBits(Uint32 scanmask) {
  assert(isInUse());
...
inline Container::Header &Container::Header::setScanInProgress() {
  assert(isInUse());
...
inline Container::Header &Container::Header::clearScanInProgress() {
  assert(isInUse());
  assert(isScanInProgress());
