...
void Dbacc::execACCFRAGREQ(Signal *signal) {
  ...
  Uint32 result =
      seizePage(spPageptr, Page32Lists::ANY_SUB_PAGE,
                c_allow_use_of_spare_pages, fragrecptr, jamBuffer());
...
void Dbacc::execACCKEYREQ(Signal *signal, Uint32 opPtrI,
  ...
  const Uint32 found =
      getElement(req, lockOwnerPtr, bucketPageptr, bucketConidx, elemPageptr,
                 elemConptr, elemptr);
...
void Dbacc::execACCKEY_ORD(Signal *signal, Uint32 opPtrI,
                           Dbacc::Operationrec *opPtrP) {
  jamEntryDebug();
...
void Dbacc::startNext(Signal *signal, OperationrecPtr lastOp) {
  jam();
  ...
  if ((opbits & Operationrec::OP_STATE_MASK) !=
...
upgrade:
   ...
   */
  {
    ...
    frp.p->m_lockStats.wait_ok(
        ((nextbits & Operationrec::OP_LOCK_MODE) != ZREADLOCK),
        nextOp.p->m_lockTime, getHighResTimer());
  }
...
void Dbacc::accIsLockedLab(Signal *signal, OperationrecPtr lockOwnerPtr) {
...
void Dbacc::insertelementLab(Signal *signal, Page8Ptr bucketPageptr,
  ...
  Uint32 conptr;
  insertElement(Element(tidrElemhead, localKey.m_page_no), operationRecPtr,
                idrPageptr, tidrPageindex, isforward, conptr,
                Operationrec::ANY_SCANBITS, false);
  release_frag_mutex_hash(fragrecptr.p, operationRecPtr);
  fragrecptr.p->m_lockStats.req_start_imm_ok(
      true /* Exclusive */, operationRecPtr.p->m_lockTime, getHighResTimer());
  c_tup->prepareTUPKEYREQ(localKey.m_page_no, localKey.m_page_idx,
                          fragrecptr.p->tupFragptr);
  sendAcckeyconf(signal);
...
Uint32 Dbacc::placeReadInLockQueue(OperationrecPtr lockOwnerPtr) const {
  ...
  do {
    ndbrequire(oprec_pool.getValidPtr(loopPtr));
    if (loopPtr.p->is_same_trans(operationRecPtr.p)) goto checkop;
    loopPtr.i = loopPtr.p->nextParallelQue;
  } while (loopPtr.i != RNIL);
...
void Dbacc::execACC_ABORTREQ(Signal *signal, Uint32 opPtrI,
  ...
  switch (sendConf) {
    ...
    case 2:
      ...
      [[fallthrough]];
    case 1:
      sendSignal(operationRecPtr.p->userblockref, GSN_ACC_ABORTCONF, signal, 1,
                 JBB);
  }
...
void Dbacc::execACC_LOCKREQ(Signal *signal) {
  ...
  if (lockOp == AccLockReq::Abort) {
    ...
    execACC_ABORTREQ(signal, operationRecPtr.i, operationRecPtr.p, 0);
    releaseOpRec();
    req->returnCode = AccLockReq::Success;
    *sig = *req;
    return;
  ...
  if (lockOp == AccLockReq::AbortWithConf) {
    ...
    execACC_ABORTREQ(signal, operationRecPtr.i, operationRecPtr.p, 1);
    releaseOpRec();
    req->returnCode = AccLockReq::Success;
    *sig = *req;
    return;
...
bool Dbacc::checkOpPendingAbort(Uint32 accConnectPtr) const {
  ...
  return ((opPtr.p->m_op_bits & Operationrec::OP_PENDING_ABORT) != 0);
}
 ...
 * S2
 */
void Dbacc::abortParallelQueueOperation(Signal *signal, OperationrecPtr opPtr) {
  jam();
...
void Dbacc::abortSerieQueueOperation(Signal *signal, OperationrecPtr opPtr) {
  jam();
...
void Dbacc::commitOperation(Signal *signal) {
  ...
  ndbrequire(((opbits & Operationrec::OP_PENDING_ABORT) == 0) ||
             (op == ZSCAN_OP) ||
             (op == ZREAD));  // Scan commits to unlock/abort
...
void Dbacc::release_lockowner(Signal *signal, OperationrecPtr opPtr,
                              bool commit) {
  OperationrecPtr nextP;
...
void Dbacc::startNew(Signal *signal, OperationrecPtr newOwner) {
  OperationrecPtr save = operationRecPtr;
  operationRecPtr = newOwner;
  ...
  if (op == ZSCAN_OP && (opbits & Operationrec::OP_LOCK_REQ) == 0) goto scan;
...
Uint32 Dbacc::seizePage(Page8Ptr &spPageptr, int sub_page_id,
  ...
  if (spPageptr.i == RNIL) {
    ...
    if (p == NULL && allow_use_of_spare_pages) {
      thrjam(jamBuf);
      p = m_ctx.m_mm.alloc_spare_page(RT_DBACC_PAGE, &ptr.i,
                                      Ndbd_mem_manager::NDB_ZONE_LE_30);
    }
    if (p == NULL) {
      thrjam(jamBuf);
...
void Dbacc::execDUMP_STATE_ORD(Signal *signal) {
  ...
  if (dumpState->args[0] == DumpStateOrd::AccDumpOneOperationRec) {
    ...
    infoEvent("fid=%d, fragptr=%d ", tmpOpPtr.p->fid, tmpOpPtr.p->fragptr);
    infoEvent("hashValue=%d", tmpOpPtr.p->hashValue.pack());
    infoEvent("nextOp=%d, nextParallelQue=%d ", tmpOpPtr.p->nextOp,
    ...
    infoEvent("prevParallelQue=%d, prevSerialQue=%d, scanRecPtr=%d",
              tmpOpPtr.p->prevParallelQue, tmpOpPtr.p->prevSerialQue,
...
Uint32 Dbacc::getL2PMapAllocBytes(Uint32 fragId) const {
  jam();
  FragmentrecPtr fragPtr(NULL, fragId);
