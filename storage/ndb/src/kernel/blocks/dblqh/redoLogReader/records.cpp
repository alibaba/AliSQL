...
Uint32 NextMbyteRecord::getLogRecordSize() { return NEXTMBYTERECORDSIZE; }
...
NdbOut &operator<<(NdbOut &no, const CommitTransactionRecord &ctr) {
  ...
  printOut("Start page index of Prep. Op.",
           ctr.m_startPageIndexOfPrepareOperation);
  printOut("Stop page no. of Prep. Op.",
           ctr.m_stopPageNumberOfPrepareOperation);
...
NdbOut &operator<<(NdbOut &no, const InvalidCommitTransactionRecord &ictr) {
  ...
  printOut("Start page index of Prep. Op.",
           ictr.m_startPageIndexOfPrepareOperation);
  printOut("Stop page no. of Prep. Op.",
           ictr.m_stopPageNumberOfPrepareOperation);
...
bool PrepareOperationRecord::check() {
  ...
  if (m_operationType == 3 && m_attributeLength != 0) return false;

  if (m_logRecordSize != (m_attributeLength + m_keyLength + 8)) return false;

  return true;
}

Uint32 PrepareOperationRecord::getLogRecordSize(Uint32 wordsRead) {
  if (wordsRead < 2) return 2;  // make sure we read more
...
NdbOut &operator<<(NdbOut &no, const PrepareOperationRecord &por) {
  ...
  switch (por.m_operationType) {
    case 0:
      ndbout_c("%-30s%-12u%-6s", "operationType:", por.m_operationType, "read");
      break;
    case 1:
      ndbout_c("%-30s%-12u%-6s", "operationType:", por.m_operationType,
               "update");
      break;
    case 2:
      ndbout_c("%-30s%-12u%-6s", "operationType:", por.m_operationType,
...
Uint32 CompletedGCIRecord::getLogRecordSize() { return COMPLETEDGCIRECORDSIZE; }
...
Uint32 PageHeader::getLogRecordSize() { return PAGEHEADERSIZE; }
...
bool PageHeader::lastPage() { return m_next_page == 0xffffff00; }

Uint32 PageHeader::lastWord() { return m_current_page_index; }
...
Uint32 FileDescriptor::getLogRecordSize() {
  return FILEDESCRIPTORHEADERSIZE + m_fdHeader.m_noOfDescriptors *
                                        NO_MBYTE_IN_FILE *
                                        FILEDESCRIPTORENTRYSIZE;
}

static const Uint32 *printFileDescriptorRecord(Uint32 no, const Uint32 *ptr) {
  ...
  for (unsigned i = 0; i < NO_MBYTE_IN_FILE; i++) {
    ndbout_c(
        ...
        i, completed[i], started[i], prepref[i], (prepref[i] >> 16),
        (prepref[i] & 65535));
  }
  ndbout << endl;
