...
Uint32 *Dbtup::alloc_fix_rowid(Uint32 *err, Fragrecord *regFragPtr,
                               Tablerec *regTabPtr, Local_key *key,
                               Uint32 *out_frag_page_id) {
  Uint32 page_no = key->m_page_no;
  Uint32 idx = key->m_page_idx;

  PagePtr pagePtr;
  if ((pagePtr.i = allocFragPage(err, regTabPtr, regFragPtr, page_no)) ==
      RNIL) {
    return 0;
  }
