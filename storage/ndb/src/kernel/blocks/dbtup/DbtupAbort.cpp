...
void Dbtup::do_tup_abort_operation(Signal *signal, Tuple_header *tuple_ptr,
                                   Operationrec *opPtrP, Fragrecord *fragPtrP,
                                   Tablerec *tablePtrP) {
  /**
  ...
  if (!(bits & Tuple_header::ALLOC)) {
     ...
     */
    jam();
    if (opPtrP->is_first_operation() && bits & Tuple_header::MM_GROWN) {
      /**
