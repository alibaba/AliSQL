...
typedef JniMemberId<WEAK_CACHING, _ByteBuffer_isReadOnly> ByteBuffer_isReadOnly;
...
typedef JniMemberId<WEAK_CACHING, _ByteBuffer_remaining> ByteBuffer_remaining;
...
typedef JniMemberId<WEAK_CACHING, _ByteBuffer_position> ByteBuffer_position;

// ---------------------------------------------------------------------------

// helper functions

inline cstatus ensureMutableBuffer(jtie_j_n_ByteBuffer jbb, JNIEnv *env);

template <jlong N>
inline cstatus ensureMinBufferSize(jtie_j_n_ByteBuffer jbb, JNIEnv *env);

inline int32_t getBufferPosition(jtie_j_n_ByteBuffer jbb, JNIEnv *env);

inline void *getByteBufferAddress(jtie_j_n_ByteBuffer jbb, JNIEnv *env);

template <typename J>
inline J *wrapAddressAsByteBuffer(const void *c, JNIEnv *env);

template <typename J>
inline J *wrapByteBufferAsReadOnly(J *jbb, JNIEnv *env);
...
struct ByteBufferPtrParam {
  static C *convert(cstatus &s, jtie_j_n_ByteBuffer j, JNIEnv *env) {
    ...
    if (j == NULL) {
      ...
      if (TypeInfo<C>::isMutable() && (ensureMutableBuffer(j, env) != 0)) {
        // exception pending
      } else {
...
template <typename C>
struct Param<jtie_j_n_ByteBuffer, C *const> : Param<jtie_j_n_ByteBuffer, C *> {
};
template <typename C>
