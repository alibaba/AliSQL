...
static const ErrStruct errArray[] = {
    ...
    {NDBD_EXIT_PARTITIONED_SHUTDOWN, XAE,
    ...
    {NDBD_EXIT_WRONG_PRIO_LEVEL, XIE,
    ...
    {NDBD_EXIT_NO_MORE_REDOLOG, XCR,
     "Fatal error due to end of REDO log. Increase NoOfFragmentLogFiles or "
     "FragmentLogFileSize"},
    ...
    */
    ...
    {NDBD_EXIT_OUT_OF_LONG_SIGNAL_MEMORY, XCR,
     ...
     "LongMessageBuffer"},
    {NDBD_EXIT_WATCHDOG_TERMINATE, XIE,
    ...
    {NDBD_EXIT_INVALID_SECRETS_FILE, XRE, "Invalid secrets file"},
    {NDBD_EXIT_MISSING_SECRETS_FILE, XRE, "Secrets file is missing"},
    ...
    /* Sentinel */
    {0, XUE,
     "No message slogan found (please report a bug if you get this error "
     "code)"}};
 ...
 */
static const StatusExitMessage StatusExitMessageMapping[] = {
    {XST_S, "Success"},
    {XST_U, "Unknown"},
...
static const int NbExitStatus =
    sizeof(StatusExitMessageMapping) / sizeof(StatusExitMessage);

static const int errArraysize = sizeof(errArray) / sizeof(ErrStruct);

static const StatusExitClassification StatusExitClassificationMapping[] = {
    {XST_S, XNE, "No error"},
    {XST_U, XUE, "Unknown"},
    {XST_R, XIE,
...
const char *ndbd_exit_message(int faultId, ndbd_exit_classification *cl) {
  int i = 0;
  while (errArray[i].faultId != faultId && errArray[i].faultId != 0) i++;
