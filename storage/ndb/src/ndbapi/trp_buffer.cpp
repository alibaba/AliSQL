...
bool TFPool::init(size_t mem, size_t reserved_mem, size_t page_sz) {
  m_pagesize = page_sz;
...
TFPool::~TFPool() {
  if (m_alloc_ptr) free(m_alloc_ptr);
}

TFMTPool::TFMTPool(const char *name) { NdbMutex_InitWithName(&m_mutex, name); }

void TFBuffer::validate() const {
