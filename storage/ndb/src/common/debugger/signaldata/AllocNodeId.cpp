...
bool printALLOC_NODEID_REQ(FILE *output, const Uint32 *theData, Uint32 len,
                           Uint16 /*recBlockNo*/) {
  ...
  switch (len) {
    case AllocNodeIdReq::SignalLength:
      fprintf(output,
              ...
              sig->senderData, sig->nodeId, sig->nodeType, sig->timeout);
      return true;
...
bool printALLOC_NODEID_CONF(FILE *output, const Uint32 *theData, Uint32 len,
                            Uint16 /*recBlockNo*/) {
  ...
  if (len == AllocNodeIdConf::SignalLength) {
    fprintf(output,
            ...
            " secret: %08x %08x\n",
            refToNode(sig->senderRef), refToBlock(sig->senderRef),
            sig->senderData, sig->nodeId, sig->secret_hi, sig->secret_lo);
    return true;
  }
  return false;
}

static const char *get_text_AllocNodeIdRef_ErrorCodes(Uint32 errorCode) {
  switch (errorCode) {
    ...
    case AllocNodeIdRef::NodeFailureHandlingNotCompleted:
      return "NodeFailureHandlingNotCompleted";
...
bool printALLOC_NODEID_REF(FILE *output, const Uint32 *theData, Uint32 len,
                           Uint16 /*recBlockNo*/) {
  ...
  if (len == AllocNodeIdRef::SignalLength) {
    fprintf(output,
            ...
            refToNode(sig->senderRef), refToBlock(sig->senderRef),
            sig->senderData, sig->nodeId, sig->errorCode,
            get_text_AllocNodeIdRef_ErrorCodes(sig->errorCode),
