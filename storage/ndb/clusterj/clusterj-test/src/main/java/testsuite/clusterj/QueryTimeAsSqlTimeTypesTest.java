...
public class QueryTimeAsSqlTimeTypesTest extends AbstractQueryTest {
    ...
    void createInstances(int number) {
        createAllTimeAsSqlTimeTypesInstances(number);
    }

    @Override
    protected void consistencyCheck(IdBase instance) {
        TimeAsSqlTimeTypes timeType = (TimeAsSqlTimeTypes) instance;
        Time time = getTimeFor(timeType.getId());
        String errorMessage = "Wrong values retrieved from ";
        errorIfNotEqual(errorMessage + "time_not_null_both", time, timeType.getTime_not_null_both());
        errorIfNotEqual(errorMessage + "time_not_null_btree", time, timeType.getTime_not_null_btree());
        errorIfNotEqual(errorMessage + "time_not_null_hash", time, timeType.getTime_not_null_hash());
        errorIfNotEqual(errorMessage + "time_not_null_none", time, timeType.getTime_not_null_none());
    }
