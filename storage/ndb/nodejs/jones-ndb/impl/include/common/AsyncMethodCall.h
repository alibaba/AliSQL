...
class AsyncCall {
 protected:
  ...
  AsyncCall(Isolate *i, Local<Function> cb) : isolate(i) {
    callback.Reset(isolate, cb);
  }

 public:
  AsyncCall(Isolate *i, Local<Value> callbackFunc) : isolate(i) {
    callback.Reset(isolate, Local<Function>::Cast(callbackFunc));
  }

  /* Destructor */
  virtual ~AsyncCall() { callback.Reset(); }
...
class AsyncCall_Returning : public AsyncCall,
 ...
 public:
  ...
  void doAsyncCallback(Local<Object> recv) override {
    ...
    if (error)
      cb_args[0] = error->toJS();
    else
      cb_args[0] = Null(AsyncCall::isolate);
