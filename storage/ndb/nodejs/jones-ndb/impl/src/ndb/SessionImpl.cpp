...
CachedTransactionsAccountant::CachedTransactionsAccountant(
    Ndb_cluster_connection *conn, int maxTransactions)
    : tc_bitmap(0),
...
inline void CachedTransactionsAccountant::tallyClear() { tc_bitmap = 0; }
...
TransactionImpl *SessionImpl::seizeTransaction(Isolate *isolate) {
  TransactionImpl *ctx;
  DEBUG_PRINT("FreeList: %p, nContexts: %d, maxNdbTransactions: %d", freeList,
              nContexts, maxNdbTransactions);
...
const NdbError &SessionImpl::getNdbError() const { return ndb->getNdbError(); }
