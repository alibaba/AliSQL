...
Envelope *getNdbDictTableEnvelope() { return &NdbDictTableEnv; }
 ...
 **
 **/
class ListTablesCall
    : public NativeCFunctionCall_2_<int, SessionImpl *, const char *> {
 private:
  Ndb *ndb;
...
class GetTableCall : public NativeCFunctionCall_3_<int, SessionImpl *,
 ...
 public:
  ...
  GetTableCall(const Arguments &args)
      : NativeCFunctionCall_3_<int, SessionImpl *, const char *, const char *>(
            NULL, args),
        ndb_table(0),
        per_table_ndb(0),
        idx_list(),
...
void GetTableCall::run() {
    ...
    */
    for (unsigned int i = 0; i < idx_list.count; i++) {
      const NdbDictionary::Index *idx =
          dict->getIndex(idx_list.elements[i].name, tableName);
      ...
      */
      ...
      if (idx_table == 0 ||
          idx_table->getObjectVersion() != ndb_table->getObjectVersion()) {
  ...
  /* List the foreign keys and keep the list around for doAsyncCallback to
   * create js objects Currently there is no listForeignKeys so we use the more
   * generic listDependentObjects specifying the table metadata object.
   */
  ...
  if (return_val == 0) {
     ...
     * that the caches are populated.
     * We only care about foreign keys where this table is the child table, not
     * the parent table.
     */
      ...
      if (fk_list.elements[i].type == NdbDictionary::Object::ForeignKey) {
        ...
        if (splitNameMatchesDbAndTable(fk.getChildTable())) {
          ...
          const NdbDictionary::Table *parent_table =
              dict->getTable(splitter.part3);
          ndb->setDatabaseName(dbName);  // back to expected value
          DEBUG_PRINT("Parent table getTable returned %s",
                      parent_table->getName());
        }
      }
    }
  } else {
...
void GetTableCall::doAsyncCallback(Local<Object> recv) {
  ...
  /* TableMetadata = {
      ...
      partitionKey     : []    ,  // ordered array of column numbers in the
    partition key sparseContainer  : null     // default column for sparse
    fields
    };
  */
  if (ndb_table && !return_val) {
    ...
    // now create the javascript foreign key metadata objects for dictionary
    // objects cached earlier
    ...
    for (unsigned int i = 0; i < fk_list.count; i++) {
      ...
      if (fk_list.elements[i].type == NdbDictionary::Object::ForeignKey) {
        ...
        if (splitNameMatchesDbAndTable(fk.getChildTable())) {
          ...
          DEBUG_PRINT("Adding foreign key for %s at %i", fk.getName(),
                      fk_number);
...
Local<Object> GetTableCall::buildDBIndex_PK() {
  ...
  SET_RO_PROPERTY(obj, SYMBOL(isolate, "name"),
                  NewUtf8String(isolate, "PRIMARY_KEY"));
  SET_RO_PROPERTY(obj, SYMBOL(isolate, "isPrimaryKey"),
...
Local<Object> GetTableCall::buildDBIndex(const NdbDictionary::Index *idx) {
  ...
  SET_RO_PROPERTY(obj, SYMBOL(isolate, "name"),
                  NewUtf8String(isolate, idx->getName()));
  SET_RO_PROPERTY(obj, SYMBOL(isolate, "isPrimaryKey"),
                  Boolean::New(isolate, false));
  SET_RO_PROPERTY(
      obj, SYMBOL(isolate, "isUnique"),
      Boolean::New(isolate,
                   idx->getType() == NdbDictionary::Index::UniqueHashIndex));
  SET_RO_PROPERTY(
      obj, SYMBOL(isolate, "isOrdered"),
      Boolean::New(isolate,
                   idx->getType() == NdbDictionary::Index::OrderedIndex));
...
*/
Local<Object> GetTableCall::buildDBForeignKey(
...
Local<Object> GetTableCall::buildDBColumn(const NdbDictionary::Column *col) {
  ...
  bool is_dec = ((col_type == NDB_TYPE_DECIMAL) ||
                 (col_type == NDB_TYPE_DECIMALUNSIGNED));
  bool is_binary =
      ((col_type == NDB_TYPE_BLOB) || (col_type == NDB_TYPE_BINARY) ||
       (col_type == NDB_TYPE_VARBINARY) ||
       (col_type == NDB_TYPE_LONGVARBINARY));
  bool is_char =
      ((col_type == NDB_TYPE_CHAR) || (col_type == NDB_TYPE_TEXT) ||
...
Local<Value> getDefaultValue(v8::Isolate *isolate,
