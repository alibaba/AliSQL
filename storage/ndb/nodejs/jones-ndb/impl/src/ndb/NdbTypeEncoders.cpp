...
Eternal<Value> /* SQLState Error Codes */
    K_22000_DataError, K_22001_StringTooLong, K_22003_OutOfRange,
    K_22007_InvalidDatetime, K_0F001_Bad_BLOB, K_HY000;
...
#define DECLARE_ENCODER_TEMPLATES(TYPE)                                       \
  template <typename T>                                                       \
  Local<Value> TYPE##Reader(const NdbDictionary::Column *, char *, uint32_t); \
  template <typename T>                                                       \
  Local<Value> TYPE##Writer(const NdbDictionary::Column *, Local<Value>,      \
                            char *, uint32_t)
...
ENCODER(SmallUnsignedEncoder, smallintReader<uint16_t>,
        smallintWriter<uint16_t>);
...
ENCODER(LongVarbinaryEncoder, varbinaryReader<uint16_t>,
        varbinaryWriter<uint16_t>);
...
struct encoder_stats_t {
  unsigned read_strings_externalized;  // JS Strings that reference ASCII or
                                       // UTF16LE buffers
  unsigned
      read_strings_created;  // JS Strings created from UTF-8 representation
  unsigned read_strings_recoded;  // Reads recoded from MySQL Charset to UTF-8
  unsigned
      externalized_text_writes;  // String reused as TEXT buffer (no copying)
...
/* FOR INTEGER TYPES, x86 allows unaligned access, but most other machines do
   not. FOR FLOATING POINT TYPES: access must be aligned on all architectures.
...
#define LOAD_ALIGNED_DATA(Type, x, buf) Type x = *((Type *)(buf));

/* Assign *buf := x, assuming correct alignment */
#define STORE_ALIGNED_DATA(Type, x, buf) *((Type *)(buf)) = (Type)x;
...
Local<Value> checkNumber(double);

template <>
inline Local<Value> checkNumber<int>(double d) {
  if (isfinite(d)) {
...
template <>
inline Local<Value> checkNumber<uint32_t>(double d) {
  if (isfinite(d)) {
    return (d >= 0 && d < 4294967296.0) ? writerOK
                                        : K_22003_OutOfRange.Get(isolate);
  }
  return K_HY000.Get(isolate);
}
...
template <typename INTSZ>
inline Local<Value> getStatusForValue(double d) {
  if (isfinite(d)) {
    return checkIntValue<INTSZ>(static_cast<INTSZ>(d))
               ? writerOK
               : K_22003_OutOfRange.Get(isolate);
  }
  return K_HY000.Get(isolate);
...
template <>
inline bool checkIntValue<int8_t>(int r) {
  return (r >= -128 && r < 128);
}

template <>
inline bool checkIntValue<uint8_t>(int r) {
  return (r >= 0 && r < 256);
}

template <>
inline bool checkIntValue<int16_t>(int r) {
  return (r >= -32768 && r < 32768);
}

template <>
inline bool checkIntValue<uint16_t>(int r) {
  return (r >= 0 && r < 65536);
}

inline Local<Value> checkMedium(int r) {
  return (r >= -8338608 && r < 8338608) ? writerOK
                                        : K_22003_OutOfRange.Get(isolate);
}
...
// Int
Local<Value> IntReader(const NdbDictionary::Column *col, char *buffer,
                       uint32_t offset) {
  LOAD_ALIGNED_DATA(int, i, buffer + offset);
  return Integer::New(isolate, i);
}

Local<Value> IntWriter(const NdbDictionary::Column *col, Local<Value> value,
                       char *buffer, uint32_t offset) {
...
// Unsigned Int
Local<Value> UnsignedIntReader(const NdbDictionary::Column *col, char *buffer,
                               uint32_t offset) {
...
template <typename INTSZ>
Local<Value> smallintReader(const NdbDictionary::Column *col, char *buffer,
                            uint32_t offset) {
...
Local<Value> smallintWriter(const NdbDictionary::Column *col,
  ...
  if (value->IsInt32()) {
    ...
    *ipos = static_cast<INTSZ>(ival);
    status =
        checkIntValue<INTSZ>(ival) ? writerOK : K_22003_OutOfRange.Get(isolate);
  } else {
    double dval = ToNumber(isolate, value);
...
Local<Value> MediumReader(const NdbDictionary::Column *col, char *buffer,
                          uint32_t offset) {
...
Local<Value> MediumWriter(const NdbDictionary::Column *col, Local<Value> value,
                          char *buffer, uint32_t offset) {
...
// Bigint encoders
template <typename T>
bool stringToBigint(const char *, T *);

template <>
bool stringToBigint<int64_t>(const char *str, int64_t *out) {
  char *endptr;
  bool rval = false;
  errno = 0;
  long long ll = strtoll(str, &endptr, 10);
  if (errno != ERANGE && ((isspace((unsigned char)*endptr) ||
                           (*endptr == '\0' && endptr != str)))) {
    *out = ll;
    rval = true;
  }
...
template <>
bool stringToBigint<uint64_t>(const char *str, uint64_t *out) {
  char *endptr;
  errno = 0;
...
template <typename T>
bool writeBigint(Local<Value>, T *);

template <>
inline bool writeBigint<int64_t>(Local<Value> val, int64_t *ipos) {
  if (val->IsInt32()) {
...
template <>
inline bool writeBigint<uint64_t>(Local<Value> val, uint64_t *ipos) {
  if (val->IsUint32()) {
    *ipos = GetInt32Value(isolate, val);
    return true;
  }
...
template <typename T>
void bigintToString(char *, T);

template <>
inline void bigintToString<int64_t>(char *strbuf, int64_t bigint) {
  sprintf(strbuf, "%lld", (long long int)bigint);
}

template <>
inline void bigintToString<uint64_t>(char *strbuf, uint64_t bigint) {
  sprintf(strbuf, "%llu", (long long unsigned int)bigint);
}

template <typename BIGT>
Local<Value> bigintReader(const NdbDictionary::Column *col, char *buffer,
                          uint32_t offset) {
  char strbuf[32];
...
template <typename BIGT>
Local<Value> bigintWriter(const NdbDictionary::Column *col, Local<Value> value,
                          char *buffer, uint32_t offset) {
...
Local<Value> DecimalReader(const NdbDictionary::Column *col, char *buffer,
                           uint32_t offset) {
  char strbuf[96];
  int scale = col->getScale();
  int prec = col->getPrecision();
  int len = scale + prec + 3;
  decimal_bin2str(buffer + offset, col->getSizeInBytes(), prec, scale, strbuf,
                  len);
  return NewUtf8String(isolate, strbuf);
}

Local<Value> DecimalWriter(const NdbDictionary::Column *col, Local<Value> value,
                           char *buffer, uint32_t offset) {
  ...
  int status =
      decimal_str2bin((const char *)strbuf, length, col->getPrecision(),
                      col->getScale(), buffer + offset, col->getSizeInBytes());
...
Local<Value> fpReader(const NdbDictionary::Column *col, char *buffer,
                      uint32_t offset) {
...
template <typename FPT>
Local<Value> fpWriter(const NdbDictionary::Column *col, Local<Value> value,
                      char *buffer, uint32_t offset) {
  double dval = ToNumber(isolate, value);
...
Local<Value> BinaryReader(const NdbDictionary::Column *col, char *buffer,
                          uint32_t offset) {
  return NewJsBuffer(isolate, buffer + offset, col->getLength());
}

Local<Value> BinaryWriter(const NdbDictionary::Column *col, Local<Value> value,
                          char *buffer, uint32_t offset) {
...
Local<Value> varbinaryReader(const NdbDictionary::Column *col, char *buffer,
                             uint32_t offset) {
...
typedef int CharsetWriter(const NdbDictionary::Column *, Local<String>, char *,
                          bool);
...
inline CharsetWriter *getWriterForColumn(const NdbDictionary::Column *col) {
  ...
  if (csinfo->isUtf8)
    ...
    writer = &writeUtf16le;
  else if (csinfo->isAscii)
    writer = &writeAscii;
  else if (csinfo->isMultibyte)
    writer = &writeRecode;
  return writer;
}
...
*/
int writeUtf16le(const NdbDictionary::Column *column, Local<String> strval,
                 char *buffer, bool pad) {
  stats.direct_writes++;
  int bufsz = column->getLength() / 2; /* Work in 16-byte characters */
  uint16_t *str = (uint16_t *)buffer;
  if (pad)
    for (int i = 0; i < bufsz; i++) str[i] = ' ';
  int charsWritten =
      strval->Write(isolate, str, 0, bufsz, String::NO_NULL_TERMINATION);
  int sz = charsWritten * 2;
  return sz;
}

int writeUtf8(const NdbDictionary::Column *column, Local<String> strval,
              char *buffer, bool pad) {
  stats.direct_writes++;
  const int &bufsz = column->getLength();
  int sz = strval->WriteUtf8(isolate, buffer, bufsz, NULL,
...
int writeAscii(const NdbDictionary::Column *column, Local<String> strval,
               char *buffer, bool pad) {
  stats.direct_writes++;
  const int &bufsz = column->getLength();
  int sz = strval->WriteOneByte(isolate, (uint8_t *)buffer, 0, bufsz,
                                String::NO_NULL_TERMINATION);
  if (pad)
    while (sz < bufsz) buffer[sz++] = ' ';
  return sz;
...
int writeGeneric(const NdbDictionary::Column *col, Local<String> strval,
                 char *buffer, bool pad) {
...
inline int recodeFromUtf8(const char *src, int srcLen, char *dest, int destLen,
                          int destCharsetNumber) {
  CharsetMap csmap;
  int32_t lengths[2] = {srcLen, destLen};
  csmap.recode(lengths, csmap.getUTF8CharsetNumber(), destCharsetNumber, src,
               dest);
  return lengths[1];
}
...
*/
int writeRecode(const NdbDictionary::Column *col, Local<String> strval,
                char *buffer, bool pad) {
  stats.recode_writes++;
  const EncoderCharset *csinfo = getEncoderCharsetForColumn(col);
  ...
  int recodeSz = strval->WriteUtf8(isolate, recode_stack, utf8bufferSize, NULL,
                                   String::NO_NULL_TERMINATION);
  if (pad) {
  ...
  int bytesWritten = recodeFromUtf8(recode_stack, recodeSz, buffer,
                                    columnSizeInBytes, col->getCharsetNumber());
  delete[] recode_stack;
  return bytesWritten;
}
...
Local<Object> getBufferForText(const NdbDictionary::Column *col,
  ...
  if (csinfo->isAscii || (valueIsAscii && !csinfo->isMultibyte)) {
    ...
    str->WriteUtf8(isolate, recode_buffer, utf8Length, 0,
                   String::NO_NULL_TERMINATION);
    int buflen = getRecodeBufferSize(length, utf8Length, csinfo);
    data = (char *)malloc(buflen);
    int result_len = recodeFromUtf8(recode_buffer, utf8Length, data, buflen,
...
Local<String> getTextFromBuffer(const NdbDictionary::Column *col,
  ...
  if (csinfo->isAscii) {
    ...
    ExternalizedUnicodeString *ext =
        new ExternalizedUnicodeString(buf, len / 2);
    ext->ref.Reset(isolate, bufferObj);
    ...
    if (csinfo->isUtf8) {
      ...
      csmap.recode(lengths, col->getCharsetNumber(),
                   csmap.getUTF8CharsetNumber(), str, recode_buffer);
...
Local<Value> CharReader(const NdbDictionary::Column *col, char *buffer,
                        uint32_t offset) {
  ...
  if (csinfo->isAscii || (!csinfo->isMultibyte &&
                          stringIsAscii((const unsigned char *)str, len))) {
    stats.read_strings_externalized++;
    while (str[--len] == ' ')
      ;     // skip past space padding
    len++;  // undo 1 place
    ExternalizedAsciiString *ext = new ExternalizedAsciiString(str, len);
    string = NewExternalOneByteString(isolate, ext);
    ...
    csmap.recode(lengths, col->getCharsetNumber(), csmap.getUTF8CharsetNumber(),
                 str, recode_buffer);
    len = lengths[1];
    while (recode_buffer[--len] == ' ') {
    };
    len++;  // skip padding, then undo 1
...
Local<Value> CharWriter(const NdbDictionary::Column *col, Local<Value> value,
                        char *buffer, uint32_t offset) {
...
template <typename LENGTHTYPE>
Local<Value> varcharReader(const NdbDictionary::Column *col, char *buffer,
                           uint32_t offset) {
  LOAD_ALIGNED_DATA(LENGTHTYPE, length, buffer + offset);
  ...
  if (csinfo->isAscii || (!csinfo->isMultibyte &&
                          stringIsAscii((const unsigned char *)str, length))) {
    stats.read_strings_externalized++;
    ...
    uint16_t *buf = (uint16_t *)str;
    ExternalizedUnicodeString *ext =
        new ExternalizedUnicodeString(buf, length / 2);
    string = NewExternalTwoByteString(isolate, ext);
...
template <typename LENGTHTYPE>
Local<Value> varcharWriter(const NdbDictionary::Column *col, Local<Value> value,
  ...
  return (strval->Length() > col->getLength())
             ? K_22001_StringTooLong.Get(isolate)
             : writerOK;
}
...
TimeHelper::TimeHelper(Local<Value> mysqlTime)
  ...
  if (mysqlTime->IsObject()) {
    Local<Value> _sign = K_sign.Get(isolate), _year = K_year.Get(isolate),
                 _month = K_month.Get(isolate), _day = K_day.Get(isolate),
                 _hour = K_hour.Get(isolate), _minute = K_minute.Get(isolate),
    ...
    if (Has(obj, _sign)) {
      sign = GetInt32(obj, _sign);
      nkeys++;
    }
    if (Has(obj, _year)) {
      year = GetInt32(obj, _year);
      nkeys++;
    }
    if (Has(obj, _month)) {
      month = GetInt32(obj, _month);
      nkeys++;
    }
    if (Has(obj, _day)) {
      day = GetInt32(obj, _day);
      nkeys++;
    }
    if (Has(obj, _hour)) {
      hour = GetInt32(obj, _hour);
      nkeys++;
    }
    if (Has(obj, _minute)) {
      minute = GetInt32(obj, _minute);
      nkeys++;
    }
    if (Has(obj, _second)) {
      second = GetInt32(obj, _second);
      nkeys++;
    }
    if (Has(obj, _microsec)) {
      microsec = GetInt32(obj, _microsec);
      nkeys++;
    }
  }
  valid = (nkeys > 0);
...
// Timstamp
Local<Value> TimestampReader(const NdbDictionary::Column *col, char *buffer,
                             uint32_t offset) {
...
*/
Local<Value> Timestamp2Reader(const NdbDictionary::Column *col, char *buffer,
                              uint32_t offset) {
...
Local<Value> Timestamp2Writer(const NdbDictionary::Column *col,
  ...
  if (valid) {
    ...
    writeFraction(col, static_cast<int>(timeMilliseconds * 1000),
                  buffer + offset + 4);
...
*/
Local<Value> DatetimeReader(const NdbDictionary::Column *col, char *buffer,
                            uint32_t offset) {
  TimeHelper tm;
...
/* Datetime2
 ...
 17 bits year*13+month  (year 0-9999, month 1-12)
  ...
  40 bits = 5 bytes
*/
Local<Value> Datetime2Reader(const NdbDictionary::Column *col, char *buffer,
                             uint32_t offset) {
  TimeHelper tm;
...
Local<Value> Datetime2Writer(const NdbDictionary::Column *col,
  ...
  if (tm.valid) {
    packedValue = 1;
    packedValue <<= 17;
    packedValue |= (tm.year * 13 + tm.month);
    packedValue <<= 5;
    packedValue |= tm.day;
    packedValue <<= 5;
    packedValue |= tm.hour;
    packedValue <<= 6;
    packedValue |= tm.minute;
    packedValue <<= 6;
...
// Year
Local<Value> YearReader(const NdbDictionary::Column *col, char *buffer,
                        uint32_t offset) {
...
Local<Value> YearWriter(const NdbDictionary::Column *col, Local<Value> value,
                        char *buffer, uint32_t offset) {
  int chkv;
  if (value->IsInt32()) {
    chkv = GetInt32Value(isolate, value);
...
Local<Value> TimeReader(const NdbDictionary::Column *col, char *buffer,
                        uint32_t offset) {
  TimeHelper tm;
  char *cbuf = buffer + offset;
...
Local<Value> TimeWriter(const NdbDictionary::Column *col, Local<Value> value,
                        char *buffer, uint32_t offset) {
  TimeHelper tm(value);
  int dtval = 0;
  if (tm.valid) {
...
/* Time2.  Uses TimeHelper.
 ...
 10 bits hour   (0-838)
  ...
  cannot be used.
*/
Local<Value> Time2Reader(const NdbDictionary::Column *col, char *buffer,
                         uint32_t offset) {
  TimeHelper tm;
  int prec = col->getPrecision();
  ...
  tm.fsp = prec;
  tm.microsec = (int)(packedValue & fsp_mask);
  packedValue >>= fsp_bits;
  tm.second = (packedValue & 0x3F);
  packedValue >>= 6;
  tm.minute = (packedValue & 0x3F);
  packedValue >>= 6;
  tm.hour = (packedValue & 0x03FF);
  packedValue >>= 10;
...
Local<Value> Time2Writer(const NdbDictionary::Column *col, Local<Value> value,
                         char *buffer, uint32_t offset) {
  TimeHelper tm(value);
  int prec = col->getPrecision();
  int fsp_size = (1 + prec) / 2;
...
// Date
Local<Value> DateReader(const NdbDictionary::Column *col, char *buffer,
                        uint32_t offset) {
  TimeHelper tm;
  char *cbuf = buffer + offset;
...
Local<Value> DateWriter(const NdbDictionary::Column *col, Local<Value> value,
                        char *buffer, uint32_t offset) {
  TimeHelper tm(value);
  int encodedDate = 0;
  if (tm.valid) {
