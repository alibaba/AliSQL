...
var path = require("path"), assert = require("assert"), fs = require("fs"),
    ...
    jones = require("database-jones"), jonesMysql = require("jones-mysql"),
...
function findMetadataScript(suiteName, suitePath, file) {
  ...
  path2 = path.join(
      jonesMysql.config.suites_dir, "standard", suiteName + "-" + file);
...
NdbMetadataManager.prototype.execDDL = function(statement, callback) {
  jones.openSession(this.sqlConnectionProperties)
      .then(
          function(session) {
...
NdbMetadataManager.prototype.createTestTables = function(
    suiteName, suitePath, callback) {
...
NdbMetadataManager.prototype.dropTestTables = function(
    suiteName, suitePath, callback) {
