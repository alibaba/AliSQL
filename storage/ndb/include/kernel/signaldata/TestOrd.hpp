...
class TestOrd {
 ...
 private:
   ...
   */
  void getSignalLoggerCommand(int no, BlockNumber &, Command &,
                              SignalLoggerSpecification &) const;
 ...
 * Set/Get test command
 */
inline void TestOrd::setTestCommand(Command cmd) {
  ASSERT_RANGE(cmd, 0, COMMAND_MASK, "TestOrd::setTestCommand");
  testCommand = cmd;
}

inline void TestOrd::getTestCommand(Command &cmd) const {
 ...
 * Set trace command
 */
inline void TestOrd::setTraceCommand(Command cmd, TraceSpecification spec) {
 ...
 * Get trace command
 */
inline void TestOrd::getTraceCommand(Command &cmd,
                                     TraceSpecification &spec) const {
 ...
 *
 */
inline UintR TestOrd::getNoOfSignalLoggerCommands() const {
 ...
 * Add a signal logger command to a specific block
 */
inline void TestOrd::addSignalLoggerCommand(BlockNumber bnr, Command cmd,
                                            SignalLoggerSpecification spec) {
 ...
 *
 */
inline void TestOrd::addSignalLoggerCommand(Command cmd,
                                            SignalLoggerSpecification spec) {
  ASSERT_RANGE(cmd, 0, COMMAND_MASK, "TestOrd::addSignalLoggerCommand");
  ASSERT_RANGE(spec, 0, LOG_MASK, "TestOrd::addSignalLoggerCommand");
 ...
 * Get Signal logger command
 */
inline void TestOrd::getSignalLoggerCommand(
    int no, BlockNumber &bnr, Command &cmd,
    SignalLoggerSpecification &spec) const {
  bnr = (BlockNumber)((signalLoggerCommands[no] >> BLOCK_NO_SHIFT) &
                      BLOCK_NO_MASK);
  cmd = (Command)((signalLoggerCommands[no] >> COMMAND_SHIFT) & COMMAND_MASK);
