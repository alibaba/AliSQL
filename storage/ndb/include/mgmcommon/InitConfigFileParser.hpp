...
class InitConfigFileParser {
 public:
  ...
  struct Context {
   ...
   public:
    void reportError(const char *msg, ...) ATTRIBUTE_FORMAT(printf, 2, 3);
    void reportWarning(const char *msg, ...) ATTRIBUTE_FORMAT(printf, 2, 3);
  };
 ...
 private:
  ...
  bool load_mycnf_groups(Vector<struct my_option> &options,
                         InitConfigFileParser::Context &ctx, const char *name,
                         const char *groups[]);
