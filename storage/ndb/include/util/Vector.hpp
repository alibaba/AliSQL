...
template <class T>
int Vector<T>::expand(unsigned sz) {
  if (sz <= m_size) return 0;
  ...
  for (unsigned i = 0; i < m_size; i++) tmp[i] = m_items[i];
  delete[] m_items;
  m_items = tmp;
  m_arraySize = sz;
  return 0;
...
Vector<T>::Vector(const Vector &src)
    : m_items(nullptr), m_size(0), m_incSize(src.m_incSize), m_arraySize(0) {
...
template <class T>
int Vector<T>::push_back(const T &t) {
...
template <class T>
int Vector<T>::push(const T &t, unsigned pos) {
...
template <class T>
T &Vector<T>::set(const T &t, unsigned pos, const T &fill_obj) {
  if (fill(pos + 1, fill_obj)) abort();
  T &ret = m_items[pos];
  m_items[pos] = t;
  return ret;
}
...
template <class T>
void Vector<T>::erase(unsigned i) {
  if (i >= m_size) abort();

  for (unsigned k = i; k + 1 < m_size; k++) m_items[k] = m_items[k + 1];
  m_size--;
}
...
template <class T>
int Vector<T>::fill(unsigned new_size, const T &obj) {
  const int err = expand(new_size);
  if (unlikely(err)) return err;
  while (m_size < new_size)
    if (push_back(obj)) return -1;
  return 0;
}
...
Vector<T> &Vector<T>::operator=(const Vector<T> &obj) {
  if (this != &obj) {
    ...
    if (unlikely(err)) abort();
    for (unsigned i = 0; i < obj.size(); i++) {
      if (push_back(obj[i])) abort();
    }
  }
  return *this;
}
...
template <class T>
int Vector<T>::assign(const T *src, unsigned cnt) {
  if (getBase() == src) return 0;  // Self-assign is a NOOP
...
template <class T>
int MutexVector<T>::expand(unsigned sz) {
  if (sz <= m_size) return 0;
...
template <class T>
T &MutexVector<T>::operator[](unsigned i) {
  if (i >= m_size) abort();
  return m_items[i];
}

template <class T>
const T &MutexVector<T>::operator[](unsigned i) const {
  if (i >= m_size) abort();
  return m_items[i];
}

template <class T>
T &MutexVector<T>::back() {
  if (m_size == 0) abort();
  return (*this)[m_size - 1];
}

template <class T>
const T &MutexVector<T>::back() const {
  if (m_size == 0) abort();
  return (*this)[m_size - 1];
}

template <class T>
int MutexVector<T>::push_back(const T &t) {
  lock();
  if (m_size == m_arraySize) {
    const int err = expand(m_arraySize + m_incSize);
    if (unlikely(err)) {
...
template <class T>
int MutexVector<T>::push_back(const T &t, bool lockMutex) {
  if (lockMutex) lock();
  if (m_size == m_arraySize) {
    const int err = expand(m_arraySize + m_incSize);
    if (unlikely(err)) {
...
template <class T>
void MutexVector<T>::erase(unsigned i) {
  if (i >= m_size) abort();
...
template <class T>
void MutexVector<T>::erase(unsigned i, bool _lock) {
  if (i >= m_size) abort();
...
template <class T>
void MutexVector<T>::clear(bool l) {
  if (l) lock();
  m_size = 0;
  if (l) unlock();
}
...
template <class T>
int MutexVector<T>::fill(unsigned new_size, const T &obj) {
  while (m_size < new_size)
    if (push_back(obj)) return -1;
  return 0;
}
