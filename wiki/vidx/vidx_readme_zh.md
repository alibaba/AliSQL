...
AliSQL 原生支持最高 16,383 维向量数据的存储及计算，集成余弦距离（COSINE）、欧氏距离（EUCLIDEAN）等主流向量运算函数，并基于深度优化的 HNSW（Hierarchical Navigable Small World）算法构建高效近邻搜索能力，支持对全维度向量列建立索引。
...
向量字段使用特殊的 [Field_vector](../../include/vidx/vidx_field.h#L30) 类型定义，继承自 [Field_varstring](../../sql/field.h#L3522)，使用二进制字符集存储浮点数数组。
...
阿里云 RDS MySQL 将向量存储作为托管能力提供，并集成数据同步、管理、备份和恢复等产品能力。RDS 的适用版本、控制台流程和参数默认值属于服务侧配置，未经核对不能直接用于本源码分支。
