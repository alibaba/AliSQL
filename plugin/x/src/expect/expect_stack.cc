...
ngs::Error_code Expectation_stack::open(const Mysqlx::Expect::Open &open) {
  ...
  for (int i = 0; i < open.cond_size(); i++) {
    ...
    const std::string &condition_value =
        cond.has_condition_value() ? cond.condition_value() : "";
...
ngs::Error_code Expectation_stack::close() {
  if (m_expect_stack.empty())
    return ngs::Error_code(ER_X_EXPECT_NOT_OPEN,
...
void Expectation_stack::post_client_stmt(const int8_t msgid,
                                         const ngs::Error_code &stmt_error) {
  if (stmt_error) post_client_stmt_failed(msgid);
}
