...
#define LOG_COMPONENT_TAG "test_sql_stored_procedures_functions"
...
static struct st_test_statement test_query_plan[] = {
    ...
    {"test", false, "CALL test.sp_one_result()"},
    {"test", false, "CALL test.sp_two_results()"},
    {"test", false, "CALL test.sp_no_results_out(@a)"},
    {"test", false, "CALL test.sp_one_result_out(@a)"},
...
static int sql_start_row(void *ctx) {
  struct st_plugin_ctx *pctx =
      (struct st_plugin_ctx *)ctx;  //  WRITE_STR("sql_start_row\n");
  DBUG_TRACE;
  pctx->current_col = 0;
  return false;
}
...
mysql_declare_plugin(test_daemon){
    ...
    "test_sql_stored_procedures_functions",
