...
static const DefaultView internal_views[] = {
	...
	// col1 "alias" is not supported yet.
    {DEFAULT_SCHEMA, "sqlite_master", "select 'table' as \"type\", table_name as \"name\", table_name as \"tbl_name\", 0 rootpage, sql from duckdb_tables union all select 'view' as \"type\", view_name as \"name\", view_name as \"tbl_name\", 0 rootpage, sql from duckdb_views union all select 'index' as \"type\", index_name as \"name\", table_name as \"tbl_name\", 0 rootpage, sql from duckdb_indexes;"},
    ...
    {"pg_catalog", "pg_indexes", "SELECT schema_name schemaname, table_name tablename, index_name indexname, NULL as \"tablespace\", sql indexdef FROM duckdb_indexes()"},
    ...
    {"pg_catalog", "pg_tables", "SELECT schema_name schemaname, table_name tablename, 'duckdb' tableowner, NULL as \"tablespace\", index_count > 0 hasindexes, false hasrules, false hastriggers FROM duckdb_tables()"},
    ...
    {"information_schema", "check_constraints", "SELECT database_name AS constraint_catalog, schema_name AS constraint_schema, constraint_name, CASE constraint_type WHEN 'NOT NULL' THEN concat(column_name, ' IS NOT NULL') ELSE constraint_text END AS check_clause FROM (SELECT dc.*, UNNEST(dc.constraint_column_names) AS column_name FROM duckdb_constraints() AS dc WHERE constraint_type IN ('CHECK', 'NOT NULL'));"},
