...
my_bool duckdb_multi_trx_in_batch = FALSE;
ulonglong duckdb_multi_trx_timeout = 5000;
my_bool duckdb_commit_multi_trx_due_to_rotate = true;
...
Relay_log_info::Relay_log_info(bool is_slave_recovery
   ...
   /* Different from 8.0, tcle_gtid_set uses global sid. */
   tcle_gtid_set(global_sid_map, global_sid_lock),
...
void Relay_log_info::decide_parallel_for_duckdb() {
  if (!duckdb_global_mode_on() || !info_thd) {
...
void Relay_log_info::cleanup_context(THD *thd, bool error)
{
  ...
  */
  if (error)
  {
    ...
    {
      thd_duckdb_context_commit_partial_batch(thd);
    }
    else
    {
...
enum_return_status Relay_log_info::add_tcle_gtid_set(const Gtid_set *gtid_set) {

  global_sid_lock->wrlock();
  enum_return_status return_status =
      this->tcle_gtid_set.add_gtid_set(gtid_set);
  global_sid_lock->unlock();
