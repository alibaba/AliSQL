...
void Prepared_statement::setup_set_params()
{
  ...
  */
  ...
  /*
    ...
    because we want to look it up in the query cache or because we want DuckDB 
    support Binary Protocol) or not.
  ...
  */
  if ((mysql_bin_log.is_open() && is_update_query(lex->sql_command) &&
      ...
      opt_general_log || opt_slow_log || duckdb_global_mode_on() ||
                                 ...
                                 uchar *packet_end)
{
  ...
  /* FOR RDS DuckDB. */
  thd->set_query(this->m_query_string);
      ...
      */
      if (query_cache.send_result_to_client(thd, thd->query()) <= 0)
      {
        ...
        */
        ...
        /* For RDS DuckDB: Different from 8.0, if there is a DuckDB table use expanded_query. */
        if (expanded_query->length()) {
          bool exist_duckdb_table = false;
          bool exist_other_table = false;
          LEX_CSTRING new_query = {expanded_query->c_ptr(), expanded_query->length()};
          lex->check_table_engine_type(exist_duckdb_table,
                                                exist_other_table);
          if (exist_duckdb_table)
            thd->set_query(new_query);
        }
