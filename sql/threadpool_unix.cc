...
#ifdef __linux__
#include <sys/epoll.h>
my_bool threadpool_workaround_epoll_bug;
...
struct thread_group_t
{
  ...
  bool shutdown;
  bool stalled;
  ulonglong total_io_count;
  ulonglong total_queue_count;
  ulonglong dump_thread_count;
...
/**
 ...
 Disassociate file descriptor with io poll descriptor
...
int io_poll_start_read(int pollfd, int fd, void *data)
{
  ...
  if (threadpool_workaround_epoll_bug)
  {
    /*
      ...
      It takes one system call more, and affects speed considerably.
    */
...
/* for dump thread only */
void tp_inc_active_thread(THD* thd, int command)
{
  DBUG_ENTER("tp_inc_active_thread");
  DBUG_ASSERT(command == COM_BINLOG_DUMP_GTID || command == COM_BINLOG_DUMP);
...
/* for dump thread only */
void tp_dec_active_thread(THD* thd, int command)
{
  DBUG_ENTER("tp_dec_active_thread");
  DBUG_ASSERT(command == COM_BINLOG_DUMP_GTID || command == COM_BINLOG_DUMP);
  connection_t *connection= (connection_t*)thd->event_scheduler.data;
  if (connection)
  {
...
inline bool connection_is_high_prio(connection_t *c)
{
  ...
  return (mode == TP_HIGH_PRIO_MODE_STATEMENTS) ||
    ...
    c->thd->mdl_context.has_explicit_locks();
}
...
static int start_timer(pool_timer_t* timer)
{
  int res= 0;
  pthread_t thread_id;
  ...
  res= mysql_thread_create(key_timer_thread,&thread_id, NULL, timer_thread, timer);
  if (res)
    sql_print_error("Can't create timer thread for threadpool (errno= %d).", res);
  DBUG_RETURN(res);
}
                               ...
                               thread_group_t *thread_group)
{
  ...
  for(;;)
  {
    ...
    thread_group->total_io_count += cnt;
...
int thread_group_init(thread_group_t *thread_group, pthread_attr_t* thread_attr)
{
  ...
  thread_group->total_io_count= 0;
  thread_group->total_queue_count= 0;
  DBUG_RETURN(0);
}
...
void get_thread_group_info(thread_group_info* thread_groups, int group_count)
{
  for (int i = 0; i < group_count; i++)
  {
    mysql_mutex_lock(&all_groups[i].mutex);
    thread_groups[i].group_id= i;
    thread_groups[i].thread_count= all_groups[i].thread_count;
    thread_groups[i].active_thread_count= all_groups[i].active_thread_count;
    thread_groups[i].connection_count= all_groups[i].connection_count;
    thread_groups[i].waiting_thread_count= all_groups[i].waiting_thread_count;
    thread_groups[i].dump_thread_count= all_groups[i].dump_thread_count;
    thread_groups[i].low_queue_count= all_groups[i].queue.elements();
    thread_groups[i].high_queue_count= all_groups[i].high_prio_queue.elements();
...
void tp_wait_begin(THD *thd, int type)
{
  ...
  DBUG_ASSERT(thd);
  DBUG_ASSERT(!(thd->get_command() == COM_BINLOG_DUMP_GTID ||
...
{
  ulong strict_trx_idle_timeout= 0;
  ...
  */
  ...
  if (strict_trx_idle_timeout && thd_is_transaction_active(c->thd))
    c->abs_wait_timeout= pool_timer.current_microtime +
      1000LL*pool_timer.tick_interval +
      1000000LL * strict_trx_idle_timeout;
  else
    c->abs_wait_timeout= pool_timer.current_microtime +
      1000LL*pool_timer.tick_interval +
...
static void *worker_main(void *param)
{
  ...
  for(;;)
  {
    ...
    thread_group->total_queue_count++;
...
bool tp_init()
{
  ...
  DBUG_RETURN(start_timer(&pool_timer)? 1: 0);
}
