...
*/
PT_package_base *Proc::PT_evoke(THD *thd, List<Item> *pt_expr_list,
...
bool Proc::send_result_metadata(THD *thd) const {
  ...
  assert(m_columns.size() > 0 && m_result_type == RESULT_SET);
...
void Sql_cmd_proc::send_result(THD *thd, bool error) {
  ...
  if (!error) {
    assert(m_proc->get_result_type() == Proc::RESULT_OK);
    my_ok(thd);
  } else {
    assert(thd->is_error());
  }
...
bool Sql_cmd_proc::check_access(THD *thd) {
  ...
  if (m_priv_type == PRIV_NONE_ACL) {
    return false;
  } else if (m_priv_type == PRIV_SUPER_ACL) {
...
bool Sql_cmd_proc::check_parameter() {
  ...
  const Proc::Parameters *current_parameters = NULL;
  Proc::Parameters_list parameters_list = m_proc->get_parameters_list();

  if (parameters_list.size() == 0) {
    current_parameters = m_proc->get_parameters();
  } else {
    for (size_t i = 0; i < parameters_list.size(); i++) {
      if (parameters_list[i]->size() == actual_size) {
        current_parameters = parameters_list[i];
        define_size = current_parameters->size();
        break;
      }
    }
  }

  if (current_parameters == NULL || actual_size != define_size) {
  ...
  if (actual_size > 0) {
    ...
    Item *item;
    while ((item = it++)) {
      if (item->field_type() != current_parameters->at(i)) {
