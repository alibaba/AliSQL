  ...
  */
  ...
  if (lex->sql_command == SQLCOM_SELECT) {
    ...
    if (exist_duckdb_table && lex->sql_command == SQLCOM_SELECT) {
      ...
      myduck_duckdb_query_and_send(thd, query, true, true);
    ...
    bool exist_duckdb_table = false;
    bool exist_other_table = false;
    if (lex->insert_table_leaf && myduck_is_duckdb_table(lex->insert_table_leaf->table)) {
    ...
    if (insert_duckdb_table) {
      if (exist_duckdb_table) {
        ...
        if (myduck_duckdb_query_update(thd, query, affected_rows)) {
          DBUG_RETURN(true);
        } else {
