...
#include "duckdb/duckdb_wrapper_c_api.h"
...
int MYSQL_BIN_LOG::new_file_impl(bool need_lock_log, Format_description_log_event *extra_description_event)
{
  ...
  if (is_relay_log && !duckdb_global_mode_on())
  {
  ...
  if (flush_and_sync(0) == 0 && can_rotate)
  {
    ...
    */
    ...
    if (!last_gtid_queued->is_empty())
    {
      ...
      /* Different from 8.0. it needs wrlock here. */
      global_sid_lock->wrlock();
...
int MYSQL_BIN_LOG::open_binlog(const char *opt_name)
{
  ...
  */
  ...
  if (duckdb_global_mode_on() && mysql_bin_log_ext.duckdb_binlog_init())
    return 1;
...

  {
    ...
    if (duckdb_global_mode_on() && mysql_bin_log_ext.duckdb_recover(log_name))
      return 1;
...
int MYSQL_BIN_LOG::prepare(THD *thd, bool all)
{
  ...
  */
  ...
  /* Here no need to check, when binlog is ON and opt_log_slave_updates is off,
   * tc_log still use MYSQL_BIN_LOG instead of TC_DUMMY_LOG, for there may be 
   * some writes is not on slave sql thread. */
  assert(thd->slave_thread ?
         (opt_log_slave_updates || duckdb_global_mode_on()) :
...
MYSQL_BIN_LOG::flush_thread_caches(THD *thd)
{
  ...
  /* 
    DUCKDB TODO: Here we need save transaction size to support multi trx
    implicit commit due to transaction size.
  */
...
MYSQL_BIN_LOG::finish_commit(THD *thd)
{
  ...
  */
  ...
  if (committed_low)
  {
    ...
    */
    assert(thd->commit_error != THD::CE_COMMIT_ERROR || duckdb_global_mode_on());
    /*
      storage engine commit
    */
...
int MYSQL_BIN_LOG::ordered_commit(THD *thd, bool all, bool skip_commit)
{
  ...
  */
  ...
  DBUG_EXECUTE_IF("crash_commit_before_log", assert(FALSE););

  if (duckdb_global_mode_on() &&
    !mysql_bin_log.is_rotating_caused_by_incident) {
    DBUG_RETURN(mysql_bin_log_ext.duckdb_commit(thd));
  }
...
static bool has_write_table_is_duckdb(TABLE_LIST *tables) {
  for (TABLE_LIST *table = tables; table; table = table->next_global) {
    /* we must do preliminary checks as table->table may be NULL */
    if (!table->is_placeholder() &&
        (table->lock_type >= TL_WRITE_ALLOW_WRITE) &&
    ...
    else
    {
      ...
      else
      {
            ...
            || (flags_write_all_set & HA_BINLOG_STMT_CAPABLE) == 0)
        {
          ...
          if (!duckdb_global_mode_on() || lex->is_stmt_row_injection() ||
...
void finish_transaction_in_engines(THD *thd, bool all, bool run_after_commit) {
  ...
  if (trn_ctx->m_flags.commit_low) {
    if (ha_commit_low(thd, all, run_after_commit))
      thd->commit_error = THD::CE_COMMIT_ERROR;
  } else if (trn_ctx->xid_state()->has_state(XID_STATE::XA_PREPARED)) {
    if (ha_rollback_low(thd, all))
      thd->commit_error = THD::CE_COMMIT_ERROR;
  }
}
}  // namespace
void trx_cache_write_event(THD *thd, Log_event *event) {
  binlog_cache_mngr *cache_mngr = thd_get_cache_mngr(thd);
  cache_mngr->trx_cache.write_event(thd, event);
}
