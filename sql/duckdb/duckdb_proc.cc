...
const LEX_STRING DUCKDB_PROC_SCHEMA = {C_STRING_WITH_LEN("dbms_duckdb")};
...
void Sql_cmd_duckdb_query::send_result(THD *thd, bool error) {
  ...
  List_iterator<Item> it(*m_list);
  input = (it++)->val_str(&str);
  ...
  protocol->start_row();
  protocol->store(result.c_str(), result.length(), system_charset_info);
