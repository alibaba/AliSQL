...
extern const LEX_STRING DUCKDB_PROC_SCHEMA;
...
class Sql_cmd_duckdb_proc_base : public Sql_cmd_admin_proc {
 public:
  explicit Sql_cmd_duckdb_proc_base(THD *thd, List<Item> *list,
                                    const Proc *proc)
...
class Sql_cmd_duckdb_query : public Sql_cmd_duckdb_proc_base {
 public:
  explicit Sql_cmd_duckdb_query(THD *thd, List<Item> *list,
                                const Proc *proc)
 ...
 public:
  explicit Duckdb_proc_query(PSI_memory_key key = 0) : Duckdb_proc_base(key) {
    ...
    Column_element elements[COLUMN_LAST] = {
        {MYSQL_TYPE_VARCHAR, C_STRING_WITH_LEN("RESULT"), 1024}};
