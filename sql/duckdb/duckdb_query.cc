...
std::unique_ptr<duckdb::QueryResult> duckdb_query(
  ...
  if (myduck::duckdb_log_options & LOG_DUCKDB_QUERY) {
    sql_print_information("my_duckdb: %s", query.c_str());
  }
  ...
  if (myduck::duckdb_log_options & LOG_DUCKDB_QUERY_RESULT) {
    if (res->HasError()) {
      sql_print_information("my_duckdb: %s", res->GetError().c_str());
    }

    sql_print_information("my_duckdb: %s", res->ToString().c_str());
  }
...
static void duckdb_send_explain_result(THD *thd, duckdb::QueryResult &result) {
  ...
  Query_result_send my_result;
  {
    ...
    field_list.push_back(new Item_empty_string("EXPLAIN FROM DUCKDB", 78, system_charset_info));
    if (my_result.send_result_set_metadata(field_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF)) {
      my_result.send_eof();
      return;
    }
  }
  ...
  {
    List<Item> field_list;
    field_list.push_back(new Item_string(plan_str.data(), plan_str.size(), system_charset_info));

    /* Prepare my_result. */
    my_result.prepare(field_list, thd->lex->unit);

    if (thd->killed) {
      thd->raise_warning(ER_QUERY_INTERRUPTED);
    }

    if (my_result.send_data(field_list)) {
      my_result.send_eof();
      return;
    }
  }
...
void duckdb_send_result(THD *thd, duckdb::QueryResult &result) {
  ...
  List<Item> visible_field_list;
  List<Item> *field_list = NULL;
  ...
  */
  // if (unit->is_simple())
  //   field_list = &(down_cast<st_select_lex *>(unit->query_term()))->fields;
  // else if (unit->set_operation()->m_is_materialized)
  //   field_list = &(unit->query_term()->query_block()->fields);
  // else
  //TODO: 5.7 resultset meta data.
  field_list = &(unit->first_select()->item_list);
  ...
  while ((item = it++))
  {
    ...
    mysql_data_types.push_back(
        result_template{item->field_type(), item->unsigned_flag, item->decimals,
        ...
        if (value.IsNull()) {
          ...
          switch (mysql_type.type) {
            ...
            case MYSQL_TYPE_BIT: {
              ...
              protocol->store(varchar, varchar_len, &my_charset_bin);
              break;
            }
            ...
            case MYSQL_TYPE_STRING: {
              if (mysql_type.cs != &my_charset_bin) {
                ...
                auto varchar = str.c_str();
                auto varchar_len = str.size();
                protocol->store(varchar, varchar_len,
                                       mysql_type.cs);
              } else {
                auto str = value.GetValueUnsafe<duckdb::string>();
                auto varchar = str.c_str();
            ...
            case MYSQL_TYPE_NEWDECIMAL: {
              ...
              auto varchar = str.c_str();
              auto varchar_len = str.size();
              protocol->store(varchar, varchar_len, system_charset_info);
              break;
            }
            ...
            case MYSQL_TYPE_FLOAT: {
              ...
              String buf;
              protocol->store(v, mysql_type.decimals, &buf);
              break;
            }
            ...
            case MYSQL_TYPE_DATETIME: {
              ...
              if (v < 0) {
                ...
                date_add_interval(&ltime, INTERVAL_MICROSECOND,
                                  interval);
              } else {
                my_micro_time_to_timeval(v, &tm);
                time_zone->gmt_sec_to_TIME(&ltime, tm);
              }
              protocol->store(&ltime, mysql_type.decimals);
              break;
            }
