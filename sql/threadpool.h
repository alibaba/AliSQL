...
struct thread_group_info
{
  ...
  longlong  low_queue_count;
  longlong  high_queue_count;
};
...
/* reduce dump thread reference in threadpool */
extern void tp_inc_active_thread(THD* thd, int command);
extern void tp_dec_active_thread(THD* thd, int command);

/* Used in SHOW for threadpool_idle_thread_count */
extern int  tp_get_idle_thread_count();
extern void get_thread_group_info(thread_group_info* thread_groups,
                                  int group_count);
...
#ifdef __linux__
extern my_bool threadpool_workaround_epoll_bug;
#endif
