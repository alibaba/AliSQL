...
#ifndef RPL_ASYNC_CONN_FAILOVER_RESET_UDF_H
#define RPL_ASYNC_CONN_FAILOVER_RESET_UDF_H

#include "sql/udf_service_impl.h"
#include "sql/udf_service_util.h"

class Rpl_async_conn_failover_reset : public Udf_service_impl {
 private:
 ...
 public:
  Rpl_async_conn_failover_reset() = default;
  ~Rpl_async_conn_failover_reset() override = default;

  /**
    Initialize variables, acquires the mysql_service_mysql_udf_metadata from the
    registry service and register the Asynchronous Connection Failover's reset
    UDFs. If there is an error registering any UDF, all installed UDFs are
    unregistered.
  ...
  */
  ...
  /**
    Resets asynchronous connection failover configuration.
  ...
  */
  static char *reset(UDF_INIT *init_id, UDF_ARGS *args, char *result,
  ...
  */
  ...
  /**
    ...
    @param[in] init_id     UDF_INIT structure
  */
  static void reset_deinit(UDF_INIT *init_id);
};
#endif /* RPL_ASYNC_CONN_FAILOVER_RESET_UDF_H */
