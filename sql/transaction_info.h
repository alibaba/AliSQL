...
public:
  ...
  */
  ...
  void move_next_to_first(Ha_trx_info *prev_ha_info, Transaction_ctx::THD_TRANS *trans)
  {
    assert(prev_ha_info->m_next != NULL);

    Ha_trx_info *cur_head = trans->m_ha_list;
    trans->m_ha_list = prev_ha_info->m_next;
    prev_ha_info->m_next = prev_ha_info->m_next->m_next;
    trans->m_ha_list->m_next = cur_head;
  }
  ...
  void move_duckdb_to_first(enum_trx_scope scope);
