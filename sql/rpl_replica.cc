...
#include "sql/duckdb/duckdb_config.h"  // myduck::global_mode
...
static void show_slave_status_metadata(mem_root_deque<Item *> *field_list,
  ...
  // field_list->push_back(new Item_empty_string("Replicate_Rewrite_Wild_Table", 29));
  field_list->push_back(
...
static bool show_slave_status_send_data(THD *thd, Master_info *mi,
  ...
  */
  ...
  protocol->store(&tmp);
  // TODO: display Replicate_Rewrite_Wild_Table, too many failed cases.
  // rpl_filter->get_rewrite_table(&tmp);
  // protocol->store(&tmp);
  // channel_name
...
QUEUE_EVENT_RESULT queue_event(Master_info *mi, const char *buf,
  ...
  */
  ...
  DBUG_EXECUTE_IF(
      "stop_when_queue_xid_event", if (event_type == binary_log::XID_EVENT) {
  ...
  */
  ...
  switch (event_type) {
    ...
    case binary_log::TRANSACTION_CONTEXT_EVENT: {
      tcle = new Transaction_context_log_event(buf,
                                               mi->get_mi_description_event());
