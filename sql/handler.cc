...
bool ha_is_storage_engine_disabled(handlerton *se_handle)
{
  ...
  if (se_handle->db_type == DB_TYPE_DUCKDB && !duckdb_global_mode_on())
    return true;
...
/* RDS begin */
static my_bool createdb_handlerton(THD *, plugin_ref plugin, void *db) {
  handlerton *hton = plugin_data<handlerton *>(plugin);
  if (hton->state == SHOW_OPTION_YES && hton->create_database)
    hton->create_database(hton, (char *)db);
  return false;
}

void ha_create_database(char *db)
{
  plugin_foreach(nullptr, createdb_handlerton, MYSQL_STORAGE_ENGINE_PLUGIN, db);
}
/* RDS end */
...
int commit_owned_gtids(THD *thd, bool all, bool *need_clear_owned_gtid_ptr)
{
  ...
  if (ending_trans(thd, all) && thd->owned_gtid.sidno <= 0 && 
      thd->multi_trx_in_batch())
  {
    if (opt_bin_log && (!thd->slave_thread || opt_log_slave_updates)) {
      duckdb_context_prepare_gtids_for_binlog_commit(thd);
    }
    error = thd_duckdb_context_save_batch_gtid_set(thd);
  }
...
*/
  ...
  else if (all && thd->multi_trx_in_batch())
  {
    thd_duckdb_context_update_on_commit(thd);
  }
...
int ha_commit_low(THD *thd, bool all, bool run_after_commit)
{
  ...
  /* DuckDB storage engine first commit. */
  if (duckdb_global_mode_on() &&
      (all || !trn_ctx->is_active(Transaction_ctx::SESSION)))
    trn_ctx->move_duckdb_to_first(trx_scope);
    ...
    */
    ...
    for (; ha_info; ha_info= ha_info_next)
    {
      ...
      if ((err= ht->commit(ht, thd, all)))
      {
        ...
        error=1;
        /* Skip committing other engines if duckdb engine failed to commit */
        if (ht->db_type == DB_TYPE_DUCKDB) DBUG_RETURN(error);
      }
      assert(!thd->status_var_aggregated);
                    ...
                    const char *db, const char *alias, bool generate_warning)
{
  ...
  /* RDS DuckDB: Fix delete table. Use alias instead of table_name in path. */
  char table_path[FN_REFLEN + 1];
  if (table_type->db_type == DB_TYPE_DUCKDB)
  {
    strncpy(table_path, path, sizeof(table_path));
    table_path[FN_REFLEN] = '\0';
    /* Start scan from the end. */
    char *ptr = strend(table_path) - 1;
    size_t table_name_len = strlen(alias);
    ...
    /* Move after '/' .*/
    ptr++;
    full_path_len += table_name_len;
    full_path_len += (ptr - table_path);
    strncpy(ptr, alias, table_name_len);
    table_path[full_path_len] = '\0';
    path = table_path;
  }
