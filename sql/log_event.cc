...
// #include "sql/rds_diagnose_log.h"
...
int Query_log_event::do_apply_event(Relay_log_info const *rli,
  ...
  if (strcmp("COMMIT", query) == 0 &&
      ...
      thd->get_duckdb_context()->duckdb_delay_commit()) {
    return 0;
  }
...
int Rotate_log_event::do_update_pos(Relay_log_info *rli) {
  ...
  /* In scenarios with high write loads and costly batch commits, we don't want
  to trigger a commit on every binlog rotates. However, there's a problem:
  not committing when binlog rotations prevents relay log purging. Therefore,
  we added a '_frequency' like parameter to perform a commit every few binlogs,
  balancing performance and relay log purge. */
  if (!is_relay_log_event() &&
      !is_artificial_event() &&  /* not a fake rotate */
      server_id != 0 && /* not a rotate generated by slave */
      rli->info_thd->multi_trx_in_batch() &&
      (rli->get_duckdb_idempotent_batch() ||
